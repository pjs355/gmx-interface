...
class CurrentPriceService {
   ...
   */
  async getCurrentPrice(marketId: string, position: 'yes' | 'no'): Promise<number | null> {
    const cached = this.cache[marketId];
    
    // Return cached data if fresh
    if (cached && Date.now() - cached[position].timestamp < this.CACHE_DURATION) {
  ...
  async refreshMarkets(marketIds: string[]): Promise<void> {
    ...
    await Promise.all(Array.from({ length: Math.min(concurrency, marketIds.length) }, () => worker()));
  }
   ...
   */
  getCacheStats(): { totalMarkets: number; liveCount: number; historicalCount: number; noneCount: number } {
    const markets = Object.values(this.cache);
    return {
      totalMarkets: markets.length,
      liveCount: markets.filter(m => m.yes.source === 'live' || m.no.source === 'live').length,
      historicalCount: markets.filter(m => m.yes.source === 'historical' || m.no.source === 'historical').length,
      noneCount: markets.filter(m => m.yes.source === 'none' && m.no.source === 'none').length
    };
  }
  ...
  private async refreshMarketPrices(marketId: string): Promise<void> {
    ...
    try {
      ...
      const orderbook = await this.orderbookService.fetchOrderbook(marketId);
      const livePrices = orderbook ? this.calculateLivePrices(orderbook) : null;
      ...
      const historicalData = historicalPrices ? this.calculateHistoricalPrices(historicalPrices) : null;

      if (livePrices && historicalData) {
        // Merge live and historical data
        this.cache[marketId] = this.mergePrices(livePrices, historicalData);
      } else if (livePrices) {
        // Only live data available, preserve existing data for missing prices
        this.cache[marketId] = this.mergeWithExisting(livePrices, existingData);
      } else if (historicalData) {
        // Only historical data available, preserve existing data for missing prices
        this.cache[marketId] = this.mergeWithExisting(historicalData, existingData);
  ...
  private calculateLivePrices(orderbook: OrderbookSnapshot): MarketPrices {
    ...
    return {
      yes: { value: yesPrice, source: yesPrice !== null ? 'live' : 'none', timestamp: Date.now() },
      no: { value: noPrice, source: noPrice !== null ? 'live' : 'none', timestamp: Date.now() }
    };
  }
   ...
   */
  private calculateHistoricalPrices(historicalPrices: Array<{
    timestamp: number;
    price: number;
    volume?: number;
  }>): MarketPrices {
    ...
    return {
      yes: { value: historicalPrice, source: 'historical', timestamp: Date.now() },
      no: { value: 1 - historicalPrice, source: 'historical', timestamp: Date.now() }
    };
  }
  ...
  private getHistoricalPrices(marketId: string): Array<{
    ...
    try {
      // Try primary ID
      let prices = predictionMarketDataService.getHistoricalPrices(marketId);
      ...
      console.warn(`Failed to get historical prices for ${marketId}:`, error);
      return null;
    }
  }
   ...
   */
  private mergePrices(live: MarketPrices, historical: MarketPrices): MarketPrices {
    return {
      yes: {
        value: live.yes.value !== null ? live.yes.value : historical.yes.value,
        source: live.yes.value !== null ? 'live' : 'historical',
        timestamp: Date.now()
      },
      no: {
        value: live.no.value !== null ? live.no.value : historical.no.value,
   ...
   */
  private mergeWithExisting(newData: MarketPrices, existingData: MarketPrices | undefined): MarketPrices {
    if (!existingData) {
      return newData;
    }
    ...
    return {
      yes: {
        value: newData.yes.value !== null ? newData.yes.value : existingData.yes.value,
        source: newData.yes.value !== null ? newData.yes.source : existingData.yes.source,
        timestamp: Date.now()
      },
      no: {
        value: newData.no.value !== null ? newData.no.value : existingData.no.value,
        source: newData.no.value !== null ? newData.no.source : existingData.no.source,
        timestamp: Date.now()
      }
    };
  }
