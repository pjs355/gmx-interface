...
class UmbrellaDataService {
   ...
   */
  async fetchQuestionsForUmbrella(umbrella: Umbrella, options?: { includeResolved?: boolean }): Promise<PredictionMarket[]> {
    ...
    const results = await Promise.all(
      umbrella.children.map(async (questionRef) => {
        try {
          const question = await predictionMarketDataService.fetchMarketById(questionRef.questionId);
          if (question && question.historicalPrices && question.historicalPrices.length > 0) {
            predictionMarketDataService.storeHistoricalPrices(
              ...
              question.historicalPrices as unknown as Array<{ timestamp?: number; ts?: number; price: number; volume?: number }>
            );
          }
          if (question) {
