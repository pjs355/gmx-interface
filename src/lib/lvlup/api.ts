...
export async function listApiKeysBySession(address: string, accessToken: string): Promise<ApiKeyInfo[]> {
  const base = getPredictionApiBaseUrl();
...
export async function listApiKeysByAddress(address: string, accessToken?: string): Promise<ApiKeyInfo[]> {
  const base = getPredictionApiBaseUrl();
  ...
  const keys = Array.isArray(json?.data?.keys) ? json.data.keys : Array.isArray(json?.keys) ? json.keys : [];
