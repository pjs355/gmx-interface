import { PredictionMarket } from './predictionMarketDataService';
...
class PredictionMarketCache {
  ...
  setMarketData(market: PredictionMarket): void {
    const historicalPrices = this.transformHistoricalPrices(market.historicalPrices || []);
      ...
      .map((item) => {
        ...
        if (typeof item === 'object' && item !== null) {
          if (item.timestamp && typeof item.timestamp === 'number') {
            ...
            const date = new Date(item.timestamp || item.time || item.date || item.ts || Date.now());
