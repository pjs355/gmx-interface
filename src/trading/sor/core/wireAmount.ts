 ...
 *   `case "levelup"`.
 */
export function wireAmountUsdForVenue(leg: Pick<RouteLeg, "venue" | "executionAmountUsd" | "fee">): number {
	const exec = Number.isFinite(leg.executionAmountUsd)
...
export function isTokenSideFeeVenue(venue: SorVenue): boolean {
	return (
		venue === "polymarket" || venue === "predictfun" || venue === "limitless"
	);
}
