import type { RouteLegBridge, SorChain } from "./sor-types";
...
export function hasFundingAddressForChain(
	...
	switch (chain) {
		case "base":
			return Boolean(
				addrs.baseSmartWallet && EVM_RE.test(addrs.baseSmartWallet),
			);
		case "polygon":
			return Boolean(
				addrs.polymarketSafe && EVM_RE.test(addrs.polymarketSafe),
			);
		case "bnb":
			return Boolean(
				addrs.embeddedEoa && EVM_RE.test(addrs.embeddedEoa),
			);
		case "solana":
			return (
		...
		case "limitlessMakerBase":
			return Boolean(
				addrs.limitlessMakerBase && EVM_RE.test(addrs.limitlessMakerBase),
			);
	}
}
...
export function chainsForBridgePrefund(opts: {
	...
	return corridor.filter((chain) =>
		hasFundingAddressForChain(chain, opts.fundingAddresses),
	);
}
