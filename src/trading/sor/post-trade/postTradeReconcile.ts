...
export type AccountReconcileSnapshot = {
	...
	venuePositionSigs: Record<AccountVenueKey, string>;
...
export function captureAccountReconcileSnapshot(input: {
	positions: Record<AccountVenueKey, { rows: VenuePosition[] }>;
	cashTotal: number;
	accountVersion: number;
	readLevelUpSide: (marketId: string, side: "yes" | "no") => number;
	levelUpMarketId: string | null;
}): AccountReconcileSnapshot {
	const venues: AccountVenueKey[] = [
		"polymarket",
		"predict",
		"dflow",
		"limitless",
	];
	const venuePositionSigs = {} as Record<AccountVenueKey, string>;
