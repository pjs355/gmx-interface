...
export function bridgeShortfallUsdToDesiredSweepMicrosFloor(
...
export function planLimitlessScwSweepMicros(
	...
	const lifiNeedUsd =
		planned > 0n ? Math.max(0, shortfall - sweepHuman) : shortfall;
...
export function recappedSweepForSend(
	...
	let capped =
		plannedSweepMicros < latestBalanceMicros
			? plannedSweepMicros
			: latestBalanceMicros;
	...
	const shortfall = Math.max(0, bridgeShortfallUsd);
	const lifiNeedUsd =
		capped > 0n ? Math.max(0, shortfall - sweepHuman) : shortfall;
	return {
