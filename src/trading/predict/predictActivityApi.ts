 ...
 */
export function normalizePredictActivityList(
	raw: unknown,
...
export function computePredictCostByTokenFromActivity(
	...
	for (const ev of events) {
		...
		if (existing) {
			...
			if (
				ms != null &&
				(existing.lastTradeAtMs == null || ms > existing.lastTradeAtMs)
			) {
...
export function predictMarketIdForTokenFromActivity(
	...
	for (const ev of events) {
		...
		if (
			ev.outcome?.onChainId &&
			normalizePredictTokenId(ev.outcome.onChainId) === want
		) {
			return Number(mid);
		}
	}
