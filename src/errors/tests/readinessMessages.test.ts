...
describe("blockingReasonToMessage", () => {
	it("maps server polymarket codes", () => {
		expect(blockingReasonToMessage("polymarket:usdc_approval_required")).toMatch(/Approve USDC/i);
	});

	it("returns fallback for unknown codes in production mode", () => {
		const prev = import.meta.env.DEV;
		(import.meta.env as { DEV: boolean }).DEV = false;
		expect(blockingReasonToMessage("unknown:code")).toBe(userMessage(READINESS_BLOCKED_FALLBACK));
