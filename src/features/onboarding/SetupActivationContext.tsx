...
const SetupActivationContext = createContext<SetupActivationContextValue | null>(null);

export function SetupActivationProvider({ children }: { children: ReactNode }) {
	const [onboardingActive, setOnboardingActive] = useState(false);
	const [venues, setVenues] = useState<Record<SetupVenueId, VenueSetupSnapshot>>({
	...
	const reportVenueSnapshot = useCallback((venue: SetupVenueId, snap: VenueSetupSnapshot) => {
	...
	return (
		<SetupActivationContext.Provider value={value}>{children}</SetupActivationContext.Provider>
	);
}
...
export function useSetupActivation(): SetupActivationContextValue {
	...
	if (!ctx) {
		throw new Error("useSetupActivation must be used within <SetupActivationProvider>");
	}
	return ctx;
}
