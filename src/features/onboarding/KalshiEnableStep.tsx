	...
	const handleEnable = useCallback(async () => {
		...
		try {
			if (!embeddedSolanaWallet) {
				throw new Error("Solana wallet not ready yet. Wait a moment and try again.");
			}
			...
			const msg = e instanceof Error ? e.message : "Couldn't start Kalshi verification.";
			setError(msg);
		} finally {
			setBusy(false);
		}
	}, [api, signMessage, embeddedSolanaWallet, queryClient, onAlreadyVerified, returnPath]);

	return (
		<div className="first-signup-setup-modal__kalshi">
			<h3 className="first-signup-setup-modal__heading">Enable Kalshi trading</h3>
			<p className="first-signup-setup-modal__sub">
				Verify your identity through DFlow to unlock Kalshi markets. You can skip and come back to
			...
			{error && <div className="first-signup-setup-modal__error">{error}</div>}
		</div>
	);
}
