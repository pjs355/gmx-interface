...
function childrenOf(u: Umbrella): Child[] {
	return ((u as { originalChildren?: Child[] }).originalChildren ?? u.children ?? []) as Child[];
}
...
export function levelUpQuestionIdsForVenueHistoryRow(
	...
	const levelUpQ = (u: Umbrella) =>
		(u.exchangeMatching as { levelup?: { questionId?: string } } | undefined)?.levelup?.questionId;

	if (pos.venue === "polymarket" && pos.conditionId?.trim()) {
		const k = polymarketConditionLookupKey(pos.conditionId);
		for (const u of umbrellas) {
			const exPoly = (u.exchangeMatching?.polymarket as { conditionId?: string } | undefined)
				?.conditionId;
			...
			const exHit = exPoly && polymarketConditionLookupKey(String(exPoly)) === k;
