...
import { Address, createPublicClient, erc20Abi, formatUnits, getAddress, http } from "viem";
import { bsc } from "viem/chains";
import { AddressesByChainId, ChainId } from "@predictdotfun/sdk";
import { BSC_RPC_URL } from "@/config/rpc";
import { predictCtfKey } from "../trade/predictContractKeys";
...
export async function readPredictOutcomeShareWei(args: {
	...
	if (!/^\d+$/.test(trimmedId)) {
		throw new Error(`readPredictOutcomeShareWei: invalid tokenId "${args.tokenId}"`);
	}
...
export function usePredictOutcomeShareOnChain(
	...
	const chainId = ChainId.BnbMainnet;
	const ctf = AddressesByChainId[chainId][predictCtfKey(isNegRisk, isYieldBearing)] as Address;
	const who = address?.startsWith("0x") ? (address as `0x${string}`) : undefined;
