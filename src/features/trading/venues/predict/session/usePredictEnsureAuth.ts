...
const predictAccount = import.meta.env.VITE_PREDICT_ACCOUNT_ADDRESS?.trim() || undefined;
	...
	useEffect(() => {
		...
		(async () => {
			...
			if (predictAccount) {
				...
				const builder = await OrderBuilder.make(ChainId.BnbMainnet, ethSigner as any, {
					predictAccount,
				});
