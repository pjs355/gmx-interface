...
describe("predictSingleMarketNativeOutcomeSide", () => {
	it("returns A when only marketIdA is set", () => {
		expect(predictSingleMarketNativeOutcomeSide({ marketIdA: "1", marketIdB: "" })).toBe("A");
	});
	it("returns B when only marketIdB is set", () => {
		expect(predictSingleMarketNativeOutcomeSide({ marketIdA: "", marketIdB: "2" })).toBe("B");
	});
});
...
describe("predictBookNeedsComplementForPosition", () => {
	it("needs complement for NO when native is A", () => {
		const m = matchedSingleMarket();
		expect(predictBookNeedsComplementForPosition(m, "yes", "Team A", "Team B")).toBe(false);
		expect(predictBookNeedsComplementForPosition(m, "no", "Team A", "Team B")).toBe(true);
	});
});
