...
export function predictExchangeKey(isNegRisk: boolean, isYieldBearing: boolean): keyof Addresses {
	if (isNegRisk) {
...
export function predictCtfKey(isNegRisk: boolean, isYieldBearing: boolean): keyof Addresses {
	if (isYieldBearing) {
		return isNegRisk
