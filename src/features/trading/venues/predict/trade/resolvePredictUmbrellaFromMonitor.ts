...
import { limitlessPositionHitsCatalogLeg } from "@/features/trading/venues/limitless/trade/limitlessCatalogTokenPair";
import { canonicalLimitlessTokenId } from "@/features/trading/venues/limitless/trade/limitlessTokenId";
import { polymarketConditionLookupKey } from "@/features/trading/venues/polymarket/trade/polymarketConditionLookup";
...
export const predictUmbrellaDebugEnabled = import.meta.env.VITE_DEBUG_PREDICT_UMBRELLA === "1";
...
export function logPredictUmbrellaOnce(
	phase: string,
	dedupeId: string,
...
export function resolvePredictUmbrellaForDisplay(
	...
	if (lookup) {
		...
		logPredictUmbrellaOnce("resolve-monitor-miss", `${tid}:${marketKey ?? "na"}`, {
			...
			closestMarketIdsInLookup: closestMarketIdLookupDiagnostics(lookup, pos.numericMarketId, 8),
			marketTitleSample: (pos.marketTitle ?? "").slice(0, 160),
			note: "No lookup hit: GET /matched-markets + umbrella.exchangeMatching lack matching predictFun token/market ids for this Predict position (ids must match Predict REST). If `closestMarketIdsInLookup` shows a nearby id (e.g. 205017 vs 205021), exchangeMatching may point at the wrong Predict market.",
		});
	} else {
	...
	logPredictUmbrellaOnce("resolve-title-miss", `${tid}:${marketKey ?? "na"}`, {
		...
		hint: "Title match only searches umbrellas returned by fetchAllUmbrellas — if the event is not in that list, use shortPredictFunMarketTitleForPortfolio in the UI.",
	});
	return null;
}
...
function catalogStubFromBatchResolve(
	...
	const name =
		pos.levelUpUmbrellaDisplayName?.trim() || pos.marketTitle?.trim() || "Resolved market";
	return {
			...
			if (venue === "predictfun") {
				...
				logPredictUmbrellaOnce(
					...
					`${resolvedCatalogId}:${normalizePredictTokenId(pos.tokenId ?? "")}:${pos.numericMarketId ?? "na"}`,
					{
						...
						numericMarketId: pos.numericMarketId,
						note: "Server levelUpUmbrellaId ignored: position token/market id not in predictFun lookup for that umbrella.",
					},
				);
			} else {
				return byId;
