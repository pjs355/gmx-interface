...
export function clampMarketSellSharesToCtfBalance(args: SellShareClampInput): SellShareClampResult {
	const planned = Number.isFinite(args.plannedShares) ? Math.max(0, args.plannedShares) : 0;
	...
	const balanceMinusDust = Math.max(0, balanceShares - CTF_DUST_MICRO_SHARES / 1_000_000);
