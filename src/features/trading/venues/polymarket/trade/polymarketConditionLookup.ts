...
export function polymarketConditionLookupKey(id: string | undefined | null): string {
	const t = (id ?? "").trim();
	if (!t) return "";
...
export function buildUmbrellaLookupByPolymarketConditionId(
	...
	for (const umb of umbrellas) {
		...
		const exPoly = (umb.exchangeMatching?.polymarket as { conditionId?: string } | undefined)
			?.conditionId;
