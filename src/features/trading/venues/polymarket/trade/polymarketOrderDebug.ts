...
function bboFromSnapshot(book: OrderbookSnapshot | null | undefined): {
	...
	const bestAsk =
		asks?.length && asks.length > 0 ? Math.min(...asks.map((a) => Number(a.price))) : null;
	const bestBid =
		bids?.length && bids.length > 0 ? Math.max(...bids.map((b) => Number(b.price))) : null;
...
function decodeOrderAmounts(message: Record<string, unknown>): {
	...
	try {
		...
		const sideLabel =
			side === "0" || side === 0 ? "BUY" : side === "1" || side === 1 ? "SELL" : String(side);
		return {
 ...
 */
export function wrapEip1193ForPolymarketDevLogging(eip1193: Eip1193Like): Eip1193Like {
		...
		request: async (args) => {
			if (args.method === "eth_signTypedData_v4" && args.params?.[1]) {
				try {
					...
					if (parsed?.primaryType === "Order" && msg && typeof msg === "object") {
...
export function logPolymarketOrderSuccessResponse(result: unknown): void {
	...
	console.info("%c[Polymarket CLOB] order accepted", "color:#16a34a;font-weight:bold", result);
}
