...
import { polyOutcomeTokenId } from "@/features/trading/venues/polymarket/trade/polyOutcomeTokenId";
...
export function findMatchedMarketByPolyConditionId(
	...
	if (!key) return null;
	return (
		markets.find((m) => polymarketConditionLookupKey(String(m.polyConditionId ?? "")) === key) ??
		null
	);
}
