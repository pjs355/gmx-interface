...
export async function readPolymarketSafeUsdceBalanceWei(safeAddress: string): Promise<bigint> {
 ...
 */
export async function readPolymarketSafePusdBalanceWei(safeAddress: string): Promise<bigint> {
	const safe = getAddress(safeAddress.trim()) as Address;
	return getPolygonPublicClient().readContract({
