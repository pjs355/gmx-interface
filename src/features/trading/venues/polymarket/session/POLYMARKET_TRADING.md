...
- **Sports/Esports taker fee** (default rate in `estimateFee`): \(C \times \text{feeRate} \times p \times (1-p)\) — see `src/features/trading/fees/polymarket.ts` and the tooltip copy.
...
**Source of truth (eventually):** `GET /api/polymarket/positions` → Polymarket Data API `/positions?user=<safe>` — proxied via predictions-api.
