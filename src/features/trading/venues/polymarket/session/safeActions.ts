...
export async function submitDepositWalletBatchWithRetries(
	...
	for (let attempt = 0; attempt <= delays.length; attempt += 1) {
		try {
			return await client.executeDepositWalletBatch(calls, walletAddress, depositWalletDeadline());
		} catch (err) {
			lastErr = err;
			const retryable = isRelayWalletNotRegisteredError(err) || isRelayWalletBusyError(err);
			if (attempt === delays.length || !retryable) {
				throw err;
			}
			if (import.meta.env.DEV) {
				const reason = isRelayWalletBusyError(err) ? "wallet busy" : "wallet not registered";
				console.debug(
...
export async function executePolygonRelayAndWait(
	...
	return withPolygonRelayMutex(async () => {
		const resp = await submitDepositWalletBatchWithRetries(client, txs, walletAddress, description);
...
export async function waitRelay(resp: RelayerTransactionResponse): Promise<string | undefined> {
	...
	if (last?.state === "STATE_FAILED") {
		const h =
			last.transactionHash?.trim() || resp.transactionHash?.trim() || resp.hash?.trim() || "";
 ...
 */
const POST_DEPLOY_BATCH_RETRY_DELAYS_MS = [1_500, 3_000, 5_000, 8_000, 13_000] as const;
...
export async function executePolymarketApprovalBatch(
	...
	const status = await checkPolymarketApprovals(walletAddress, chainRead);
	if (status.allApproved) return;
	const txs = buildPolymarketApprovalTransactions();
	await executePolygonRelayAndWait(client, txs, walletAddress, "LevelUp Polymarket approvals");
}
