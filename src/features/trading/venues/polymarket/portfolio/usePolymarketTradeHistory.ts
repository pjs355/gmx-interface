...
function activityShares(row: PolymarketActivityRow): number {
	...
	return numFromApi(r.size) || numFromApi(r.tokens) || numFromApi(r.amount);
}

/** USDC moved in the activity (spent/received/redeemed) */
function activityUsdc(row: PolymarketActivityRow): number {
	const r = row as unknown as Record<string, unknown>;
	return numFromApi(r.usdcSize) || numFromApi(r.cash) || numFromApi(r.usdc);
}
...
function activityRowToFill(row: PolymarketActivityRow): VenueHistoryFill | null {
	...
	const price = pr != null && Number.isFinite(pr) && pr > 0 ? pr : null;
...
function redeemRowToFill(r: PolymarketActivityRow): VenueHistoryFill | null {
	...
	const src =
		...
		undefined;
	const price = tok > 0 && usd > 0 ? usd / tok : null;
...
async function fetchActivityPage(
	...
	const rows = await api.getPolymarketActivity({ type, limit, offset });
...
function accumulateOrphanRedeemTotals(redeems: PolymarketActivityRow[]): OrphanTotals {
	...
	for (const r of redeems) {
		...
		const cur = totals.get(cid) ?? { usdcSum: 0, lastActivityMs: null };
		cur.usdcSum += usd;
...
function distributeOrphanPolymarketRedeems(
	...
	for (const [cid, { usdcSum, lastActivityMs }] of orphanTotals) {
		...
		const tradedAtIso = lastActivityMs != null ? new Date(lastActivityMs).toISOString() : "";
...
/**
 * Paginates TRADE + REDEEM activity via predictions-api → Polymarket Data API.
 */
async function fetchPolymarketTradeHistoryPages(
	api: PrivateApiClient,
	type: "TRADE" | "REDEEM",
	limit: number,
	maxPages: number,
	...
	for (let page = 0; page < maxPages; page++) {
		const rows = await fetchActivityPage(api, type, limit, offset);
...
/**
 * Fetches TRADE + REDEEM activity for the authenticated user's Polymarket wallet and aggregates
 * into per-market history entries. REDEEM presence determines win status.
 */
async function fetchPolymarketTradeHistory(api: PrivateApiClient): Promise<VenuePosition[]> {
	const [trades, redeems] = await Promise.all([
		fetchPolymarketTradeHistoryPages(api, "TRADE", 500, 40),
		fetchPolymarketTradeHistoryPages(api, "REDEEM", 500, 24),
	]);
	...
	for (const agg of byKey.values()) {
		agg.fills.sort((a, b) => Date.parse(a.tradedAt || "0") - Date.parse(b.tradedAt || "0"));
	}

	const results = Array.from(byKey.values()).map((agg): VenuePosition => {
		const avgPrice = agg.totalBought > 0 ? agg.totalSpent / agg.totalBought : null;
		...
		return {
			...
			currentValue: 0,
			pnl,
			pnlPercent: agg.totalSpent > 0 ? (pnl / agg.totalSpent) * 100 : null,
			tokenId: agg.asset,
...
/**
 * Fetches comprehensive Polymarket trade history via private API proxy of the Data API activity feed.
 ...
 */
export function usePolymarketTradeHistory(safeAddress: string | undefined | null) {
	...
	return useQuery<VenuePosition[]>({
		...
		staleTime: 120_000,
		queryFn: () => fetchPolymarketTradeHistory(api),
	});
}
