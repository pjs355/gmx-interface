...
import { clampMarketSellSharesToCtfBalance } from "@/features/trading/venues/polymarket/trade/polymarketSellShareClamp";
...
export async function executeLeg(input: VenueLegDispatchInput): Promise<SorLegResult> {
	...
	const { polyClob, matchedMonitor, ensureTokenApprovals, getRelayClient } = deps;
	...
	const approvalErr = await runSorTokenApprovalsGate({
		...
		polymarketTrackApprovalWork: true,
	});
	if (approvalErr) {
		return {
			filled: false,
	...
	const tickStyle =
		matchedMonitor?.polyTickSize != null ? (matchedMonitor.polyTickSize as TickSize) : undefined;
	const negRisk =
		matchedMonitor?.polyNegRisk != null ? Boolean(matchedMonitor.polyNegRisk) : undefined;
	...
	const attemptWrapAndPlace = async (): Promise<{
			...
			if (usdceWei > 0n) {
				...
				if (!relayClient) {
					return {
						filled: false,
						filledShares: 0,
						error: "Polymarket relayer unavailable — cannot wrap USDC.e to pUSD before trading.",
					};
				}
		...
		if (side === "buy") {
			const wire = await resolvePostBridgeMarketBuyWire({
				...
				readWalletUsd: async () => {
					const pusdWei = await readPolymarketSafePusdBalanceWei(rawSafe);
					return Number(formatUnits(pusdWei, 6));
				},
			});
			if (!wire.ok) {
				return { filled: false, filledShares: 0, error: wire.error };
			}
			buyAmountUsd = wire.amountUsd;
			postBridgeScale = wire.scale;
		}
			...
			try {
				...
				return {
					...
					error: formatErrorForUser(
						e instanceof Error ? e : new Error(userMessage(POLYMARKET_CTF_BALANCE_READ_FAILED)),
					),
				};
			}
			const tickNumeric = typeof tickStyle === "string" ? Number(tickStyle) : undefined;
		...
		return {
			filled: true,
			filledShares: side === "buy" ? leg.shares * postBridgeScale : leg.shares * sellScale,
		};
	};
	...
	try {
		...
		if (isPolymarketAllowanceRecoverableError(msg) && ensureTokenApprovals) {
			console.debug("[SOR][polymarket] allowance error, attempting one-shot recovery", {
			...
			try {
				...
				return {
					filled: false,
					filledShares: 0,
					error: formatPolymarketApprovalRepairFailed(formatErrorForUser(recovErr)),
				};
			} finally {
				if (didRecoveryWork) {
