...
export function limitlessPositionHitsCatalogLeg(
	...
	if (slug && ((oa && sLo === oa.toLowerCase()) || (ob && sLo === ob.toLowerCase()))) return true;
	const posGroup = String(pos.limitlessGroupSlug ?? "").trim();
	const gSlug = String(lx.groupSlug ?? "").trim();
	if (posGroup && gSlug && posGroup.toLowerCase() === gSlug.toLowerCase()) return true;
	return false;
}
...
export function inferLimitlessYesColumnFromOutcomeTitle(
	...
	return o === "yes" || (o !== "no" && titleLo.includes(o));
}
