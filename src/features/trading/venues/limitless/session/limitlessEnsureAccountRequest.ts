...
export async function postLimitlessEnsureAccountWhenNeeded(
	...
	postEnsure: (body?: Record<string, unknown>) => Promise<unknown>,
): Promise<unknown> {
	const ownerId =
		readLimitlessOwnerIdFromEnsurePayload(cachedEnsureData) ??
		readLimitlessOwnerIdFromEnsurePayload(queryClient.getQueryData(ensureQueryKey));
	const body = ownerId == null ? await buildEoaBody().catch(() => undefined) : undefined;
