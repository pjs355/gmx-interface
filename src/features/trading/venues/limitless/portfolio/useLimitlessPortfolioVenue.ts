...
import { getLimitlessVenueBucket } from "@/features/trading/venues/limitless/portfolio/splitLimitlessVenuePositions";
...
import { limitlessQueryKeys } from "../trade/limitlessQueryKeys";
import { canonicalLimitlessTokenId } from "../trade/limitlessTokenId";
...
function limitlessNegRiskWireFromPositionsRow(o: Record<string, unknown>): boolean {
	const market =
		o.market && typeof o.market === "object" ? (o.market as Record<string, unknown>) : undefined;
	...
	const nonEmptyMkt = (rec: Record<string, unknown> | undefined) => {
		if (!rec) return false;
		for (const k of ["negRiskMarketId", "neg_risk_market_id", "negRiskMarketID"] as const) {
			const v = rec[k];
			if (typeof v === "string" && v.trim() !== "") return true;
		}
		return false;
	};
	return nonEmptyReq(market) || nonEmptyReq(o) || nonEmptyMkt(market) || nonEmptyMkt(o);
}
...
function normalizeBytes32HexLoose(raw: string): string {
	const t = raw.trim();
	if (!t) return "";
	const with0x = /^0x/i.test(t) ? t : /^[a-fA-F0-9]{64}$/i.test(t) ? `0x${t}` : "";
...
function negRiskParentConditionIdFromPositionsRow(
	...
	const group =
		o.group && typeof o.group === "object" ? (o.group as Record<string, unknown>) : null;
	const market =
		o.market && typeof o.market === "object" ? (o.market as Record<string, unknown>) : null;
	const nestedGroup =
...
function isLimitlessRouteUnavailable(err: unknown): boolean {
	return err instanceof PrivateApiError && (err.status === 501 || err.status === 405);
}
...
function mapPositionsVenueRow(raw: unknown): VenuePosition | null {
	...
	const limitlessPartnerRedeemableSignal =
		sigRaw === "omit" || sigRaw === "true" || sigRaw === "false" ? sigRaw : undefined;
	const posOut: VenuePosition = {
		venue: "limitless",
		marketTitle: str(o.marketTitle) || slug || venueDisplayLabel("limitless"),
		outcome: str(o.outcome) || "Yes",
		shares,
		avgPrice: o.avgPrice === null || o.avgPrice === undefined ? null : num(o.avgPrice),
		currentPrice:
			o.currentPrice === null || o.currentPrice === undefined ? null : num(o.currentPrice),
		...
		pnlPercent: o.pnlPercent === null || o.pnlPercent === undefined ? null : num(o.pnlPercent),
		tokenId,
		...
		...(limitlessPartnerRedeemableSignal ? { limitlessPartnerRedeemableSignal } : {}),
...
function mapHistoryRowToVenuePosition(row: unknown): VenuePosition | null {
	...
	const tokenId = canonicalLimitlessTokenId(
		str(src.tokenId ?? src.asset ?? src.positionId ?? src.outcomeTokenId ?? src.token_id),
	);
	if (!tokenId) return null;
	...
	const usdc = usdcHumanFromHistorySrc(src, usedAmountForShares);
	...
	const marketStatus =
		str(
			src.marketStatus ?? src.status ?? src.state ?? marketObj?.status ?? marketObj?.marketStatus,
		) || undefined;
...
async function fetchLimitlessTradeHistoryPages(
	getHistory: (q: { limit: number; cursor?: string | null }) => Promise<unknown>,
	...
	for (let i = 0; i < maxHistoryPages; i++) {
		...
		if (import.meta.env.DEV && i === 0 && rows[0]) {
			debugLimitlessShallowRowShape("GET portfolio/history first page first raw row", rows[0]);
		}
		let added = 0;
		for (const r of rows) {
			...
			const rowId = str(ro.id ?? ro.tradeId ?? ro.transactionId ?? ro.txHash ?? ro.orderId);
		...
		const next = typeof rawNext === "string" && rawNext.trim().length > 0 ? rawNext.trim() : null;
		if (!next) break;
		cursor = next;
	}
...
export function useLimitlessTradeHistory(enabled: boolean) {
	...
	return useQuery({
		...
		queryFn: async () => {
			try {
				return await fetchLimitlessTradeHistoryPages((q) => api.getLimitlessPortfolioHistory(q));
			} catch (e) {
				if (isLimitlessRouteUnavailable(e)) return [];
				throw e;
			}
