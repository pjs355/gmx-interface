...
import {
	fetchLimitlessApprovalsChainRead,
	fetchLimitlessFokBuyUsdcPreflightChainRead,
} from "@/features/trading/chain-reads/limitlessChainRead";
...
async function withLimitlessApprovalLock<T>(maker: string, fn: () => Promise<T>): Promise<T> {
...
export async function readLimitlessBuyUsdcAllowancesSufficientOnBase(opts: {
	...
	const result = await fetchLimitlessApprovalsChainRead(opts.chainRead, {
		maker: opts.maker,
		verify: opts.verify,
	});
...
export async function readLimitlessMakerUsdcPreflightForFokBuy(opts: {
	...
	const result = await fetchLimitlessFokBuyUsdcPreflightChainRead(opts.chainRead, {
		maker: opts.maker,
		verify: opts.verify,
		wireUsd: opts.wireUsd,
		feeUsd: opts.feeUsd,
	});
	return {
		ok: result.ok,
		reason: result.reason,
		balanceMicro: BigInt(result.balanceMicro),
		needMicro: BigInt(result.needMicro),
		minAllowanceMicro: BigInt(result.minAllowanceMicro),
 ...
 * - **Sell:** `setApprovalForAll` on the **underlying** Gnosis CTF. Operators come from
 *   `POST /chain/read` (`venue: limitless`, `kind: approvals`).
 *
 * Each missing approval is its own `sendTransaction({ to, data, value, chainId })` then
 * receipt wait — no mixed USDC+CTF batches.
 *
 * Approval **reads** route through the private API (`POST /chain/read`).
 *
...
async function ensureLimitlessTradingApprovalsOnBaseInner(opts: {
	...
	if (!opts.verify.spender?.trim() || !ethers.isAddress(opts.verify.spender.trim())) {
		throw new Error(
	...
	const approvalRead = await fetchLimitlessApprovalsChainRead(opts.chainRead, {
		maker,
		verify: opts.verify,
	});

	if (opts.side === "buy") {
		for (const { spender, sufficient } of approvalRead.usdcSpenderReads) {
		...
		if (approvalRead.resolvedCtfAddress) {
			resolvedCtf = ethers.getAddress(approvalRead.resolvedCtfAddress) as `0x${string}`;
		}
		for (const { operator, approved } of approvalRead.ctfOperatorReads) {
	...
	for (let i = 0; i < calls.length; i += 1) {
		...
		try {
			await waitForBaseTransactionSuccess(hash, `Limitless ${opts.side} approval ${step}`);
		} catch (err) {
