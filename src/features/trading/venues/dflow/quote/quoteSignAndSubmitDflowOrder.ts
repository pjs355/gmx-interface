...
export type DflowOrderSubmitFn = (body: DflowOrderSubmitBody) => Promise<DflowOrderSubmitResponse>;
...
export async function quoteSignAndSubmitDflowOrder(args: {
	...
	const { privateApi, submitFn, solanaSigner, orderParams, submitExtras } = args;
	...
	if (orderResult.code || orderResult.msg) {
		throw new Error(mapDflowOrderError(orderResult.code, orderResult.msg));
	}
	...
	if (typeof lvbh !== "number" || !Number.isFinite(lvbh) || lvbh <= 0 || !Number.isInteger(lvbh)) {
	...
	const signedBytes = await solanaSigner.signTransactionOnly(transaction.serialize());
	const signedBase64 =
