...
function dflowEventGroupKey(market: DflowBatchMarket): string {
	const et = dflowEventTickerFromBatchMarket(market) ?? market.eventTicker?.trim() ?? "";
...
export function patchDflowVenuePositionOutcomes(
	...
	const next = rows.map((row) => {
		...
		const bucket: "Yes" | "No" = outcome.trim().toLowerCase() === "no" ? "No" : "Yes";
		let umb: Umbrella | undefined = tid && mintUmb ? mintUmb.get(tid) : undefined;
		if (!umb && row.dflowEventTicker?.trim() && etMap && umbrellasEt?.length) {
			umb =
				lookupUmbrellaByDflowEventTicker(row.dflowEventTicker.trim(), etMap, umbrellasEt) ??
				undefined;
		}
		const colLabels = portfolioColumnTeamLabels(umb);
		const dflowTradeSideLabel = bucket === "Yes" ? colLabels.columnYes : colLabels.columnNo;

		if (outcome === row.outcome && dflowTradeSideLabel === row.dflowTradeSideLabel) {
			return row;
		}
 ...
 * for avgPrice.
 */
export function buildCostMap(trades: DflowOnchainTrade[]): Map<string, CostEntry> {
	const buckets = new Map<string, CostBucket>();

	for (const t of trades) {
		if (!t.outputMint) continue;
		const shares = t.contracts ?? t.outputAmount ?? 0;
		const cost = t.usdPricePerContract != null ? t.usdPricePerContract * shares : 0;
...
export function buildDflowHistoryFillsByMint(
	...
	for (const t of trades) {
			...
			if (shares > 0) {
				const usdc =
					t.usdPricePerContract != null && Number.isFinite(t.usdPricePerContract)
						...
						: 0;
				const price = dflowFillProbPrice(usdc, shares, t.usdPricePerContract);
		...
		if (
			...
			const hasContracts = t.contracts != null && Number.isFinite(t.contracts) && t.contracts !== 0;
			const shares = hasContracts ? Math.abs(t.contracts as number) : Math.abs(t.inputAmount ?? 0);
 ...
 */
export function collectOutcomeMintCandidatesFromTrades(trades: DflowOnchainTrade[]): string[] {
...
export function toVenuePositions(
	...
	return positions.map((pos) => {
		...
		const isWon = settlementKnown && winnerContract !== null && winnerContract === pos.side;
		...
		return {
			...
			outcomeResult: settlementKnown && winnerContract !== null ? (isWon ? "WON" : "LOST") : null,
			...(pos.market.yesSubTitle?.trim()
				? { dflowYesSubTitle: pos.market.yesSubTitle.trim() }
				: {}),
			...(pos.market.noSubTitle?.trim() ? { dflowNoSubTitle: pos.market.noSubTitle.trim() } : {}),
			...(latestMs != null ? { historyTradeAt: new Date(latestMs).toISOString() } : {}),
