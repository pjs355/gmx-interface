...
import { isDflowCatalogDriftIgnoredMint } from "../catalog/dflowCatalogDriftIgnoredMints";
 ...
 *      Dev **RAW** logs for what Positions consumes: [`useDflowBundle`](LevelUp_Predictions/src/features/positions/hooks/venues/dflow/useDflowBundle.ts) (`[DFlow positions][Positions UI]`).
...
export function useDflowPositions(
	...
	const applyCatalogOutcomes = useCallback(
		...
		[catalogColumnMap, dflowOutcomeMintToUmbrella, dflowEventTickerLookup, umbrellas],
	);

	return useQuery<VenuePosition[]>({
		queryKey: ["dflow-positions", solanaAddress?.trim() ?? null, dflowMintCatalogSig],
		...
		queryFn: async () => {
			...
			const mark = (label: string) => {
				if (debugPerf) {
					console.log(`[DFlowPerf] ${label}: ${(performance.now() - t0).toFixed(0)}ms`);
				}
			};
			...
			const expandedCandidates = expandDflowMintsWithCoListedLegs(tradeMintCandidates, umbrellas);
			let outcomeMints =
			...
			if (import.meta.env.DEV) {
				...
				for (const t of positive) {
					...
					if (!catalogWire.has(mint) && !isDflowCatalogDriftIgnoredMint(mint)) {
						console.warn(
			...
			const recovered = marketPositionsForUnmatchedTokens(outcomeTokens, matchedMints, markets);
