...
export function buildTradePreview(input: BuildTradePreviewInput): TradeQuote {
	...
	const dflowSorSingleLegMarketBuy = Boolean(
		sr && sr.legs.length === 1 && sr.legs[0].venue === "dflow" && side === "buy",
	);
	const hasSorData =
		Boolean(sorMatchesInput) &&
		(tradingVenue !== "all" || (tradingVenue === "all" && dflowSorSingleLegMarketBuy));

	if (hasSorData && orderType === "market" && sr) {
		const leg = sr.legs[0];
		const shareVenueCfg = getVenueConfig(tradingVenue);
		const netBuyShares =
			side === "buy" ? sorBuyNetHeldTotalSharesFromLegs(sr.legs, predictFunFeeRateBps) : null;
		const sorContractsRaw =
			side === "buy" && netBuyShares != null
				...
				: shareVenueCfg.requiresWholeShares
					...
					: sr.totalShares;
		const sorContracts = Number.isFinite(sorContractsRaw) ? sorContractsRaw : undefined;
		...
		if (side === "buy") {
			const preview: TradePreviewFields = {
				...
				spent: sorCost !== undefined && sorFee !== undefined ? sorCost - sorFee : bookData.spent,
