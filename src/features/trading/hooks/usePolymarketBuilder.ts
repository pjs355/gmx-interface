...
export function usePolymarketBuilder(options: UsePolymarketBuilderOptions = {}) {
	...
	const verifyOnChain = useMutation({
		mutationFn: (body?: PolymarketVerifyOnChainBody) => api.postPolymarketVerifyOnChain(body ?? {}),
		onSuccess: async () => {
			await qc.invalidateQueries({
				queryKey: tradingQueryKeys.polymarketAccount,
			});
	...
	const l2Credentials = useMutation({
		mutationFn: (body: PolymarketL2CredentialsBody) => api.postPolymarketL2Credentials(body),
		onSuccess: async () => {
			await qc.invalidateQueries({
				queryKey: tradingQueryKeys.polymarketAccount,
			});
	...
	const builderSign = useMutation({
		mutationFn: (body: PolymarketBuilderSignBody) => api.postPolymarketBuilderSign(body),
	});
