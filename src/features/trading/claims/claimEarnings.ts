...
import { useWallets as usePrivyWallets, useSendTransaction } from "@privy-io/react-auth";
import {
...
import {
	...
	POLYGON_PUSD,
	POLYGON_USDC_E,
} from "@/features/trading/venues/polymarket/trade/constants";
...
import { readPolymarketSafeCtfBalanceWei } from "@/features/trading/venues/polymarket/trade/polygonCollateralWrap";
import { encodePacked, keccak256, type Address, type Hex } from "viem";
import { predictCtfKey } from "@/features/trading/venues/predict/trade/predictContractKeys";
import {
	ensurePredictChain,
	getBscBrowserSigner,
} from "@/features/trading/venues/predict/wallet/bnbWallet";
...
import { redeemLimitlessWinningPositionOnBase } from "@/features/trading/venues/limitless/portfolio/limitlessRedeemOnBase";
...
function parseLimitlessOwnerIdFromEnsureData(data: unknown): number | undefined {
	...
	const inner =
		d.data != null && typeof d.data === "object" ? (d.data as Record<string, unknown>) : d;
...
function parseLimitlessMakerAddressFromEnsureData(data: unknown): string | undefined {
	...
	if (typeof signerAddr === "string" && signerAddr.trim() && ethers.isAddress(signerAddr.trim())) {
 ...
 */
const NEG_RISK_ADAPTER_ABI = ["function redeemPositions(bytes32 _conditionId, uint256[] _amounts)"];
...
async function readPredictOutcomeTokenBalance(args: {
	...
	const ctfAddr = AddressesByChainId[chainId][predictCtfKey(args.isNegRisk, args.isYieldBearing)];
	const collateral = AddressesByChainId[chainId].USDT;
	const ctf = new Contract(ctfAddr, CTF_READ_ABI, args.provider);
	const collectionId = await ctf.getCollectionId(ethers.ZeroHash, args.conditionId, args.indexSet);
 ...
 */
function computeStandardCtfCollectionId(conditionId: Hex, indexSet: bigint): Hex {
	return keccak256(encodePacked(["bytes32", "uint256"], [conditionId, indexSet]));
}

function computeStandardCtfPositionId(collateralToken: Address, collectionId: Hex): bigint {
 ...
 */
const POLYMARKET_STANDARD_REDEEM_COLLATERALS: readonly Address[] = [POLYGON_PUSD, POLYGON_USDC_E];
...
type MarketVenue = "levelup" | "polymarket" | "predictfun" | "dflow" | "limitless";
 ...
 */
export function useClaimForVenue(market: PredictionMarket, resolvedOutcome: "yes" | "no") {
	const { account, hasSmartWallet, signer, signerAddress } = useSignerContext() as any;
	...
	const claim = useCallback(async () => {
		...
		async function redeemDflow(): Promise<string> {
			if (!embeddedSolanaWallet) {
				throw new Error("Connect your Solana wallet to redeem Kalshi / DFlow winnings");
			}
			...
			if (!Number.isFinite(shares) || shares <= 0) {
				throw new Error("No redeemable Kalshi shares on this row — refresh the page and try again");
			}
		...
		async function redeemPolymarket(): Promise<string | undefined> {
			...
			if (!relayClient)
				throw new Error("Polymarket wallet not ready — connect your wallet and try again");
			...
			const polyAssetTokenId = String(polyMarket._polyAssetTokenId ?? "").trim();
			const polyIsNegRisk = polyMarket._isNegRisk === true;

			async function assertPolymarketOutcomeBurned(lastTxHash: string | undefined): Promise<void> {
			...
			if (polyIsNegRisk) {
				...
				if (balanceWei <= 0n) {
					throw new Error("No redeemable Polymarket tokens on your deposit wallet for this market");
				}
				const amounts: [bigint, bigint] =
					resolvedOutcome === "yes" ? [balanceWei, 0n] : [0n, balanceWei];
			 ...
			 */
			if (!polyAssetTokenId) {
				throw new Error("Polymarket claim missing on-chain asset id — refresh and try again");
			}
			...
			const collateralsToTry = polymarketCtfRedeemCollateralOrder(detectionHint);
		...
		async function redeemPredict(): Promise<string> {
			...
			if (isNegRisk) {
				...
				if (amount === 0n) {
					throw new Error("No redeemable outcome tokens for this market on your Predict account");
				}
			}
			...
			const hash =
				receipt?.hash ?? (receipt as { transactionHash?: string } | undefined)?.transactionHash;
			if (!hash) {
		...
		async function redeemLevelUp(): Promise<string> {
			const indexSet = resolvedOutcome === "yes" ? YES_INDEX_SET : NO_INDEX_SET;
		...
		async function redeemLimitless(): Promise<string> {
			...
			if (!profileId) {
				throw new Error("Profile not loaded yet — wait a moment and try Claim again.");
			}
			if (!signer) {
				throw new Error("Connect your Limitless trading wallet to redeem on Base.");
			}
			...
			const limitlessMaker = parseLimitlessMakerAddressFromEnsureData(ensureData);
			let signerAddr = "";
