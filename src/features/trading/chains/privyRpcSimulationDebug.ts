...
export function inspectTransferCheckedInstructions(serializedTx: Uint8Array):
	| {
			ok: true;
	...
	try {
		...
		const hasLut =
			...
			Array.isArray((msg as { addressTableLookups?: unknown[] }).addressTableLookups) &&
			((msg as { addressTableLookups?: unknown[] }).addressTableLookups?.length ?? 0) > 0;
