...
import { formatPrivySponsoredSolanaFailureBlock } from "@/features/trading/chains/privyRpcSimulationDebug";
...
const BLOCKHASH_ERROR_PATTERNS = [/blockhash not found/i, /block ?hash/i, /expired/i];
...
export async function sendPrivySponsoredSolanaTransaction(
	...
	for (let attempt = 0; attempt < BLOCKHASH_RETRY_ATTEMPTS; attempt++) {
		...
		try {
			...
			if (typeof console !== "undefined") {
				console.warn(`[privySponsoredSolana] send failed (attempt ${attempt}): ${msg}`);
				console.warn(
