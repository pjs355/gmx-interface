/**
 * LevelUp venue fee math — backend parity required.
 *
 * Mirrors `calculateBuyFee` in predictions-api (`order-validation.ts` / CTF settlement).
 * Used by `venueConfig`, LevelUp SOR execute/signing, and quote preview.
 */
...
export function levelUpBuyFeeMicroFromMakerMicro(makerAmountMicro: bigint): bigint {
	const feeBeforeRounding = (makerAmountMicro * FEE_RATE_PERCENT_BI) / 100n;
	return ((feeBeforeRounding + MICRO_PER_CENT_BI - 1n) / MICRO_PER_CENT_BI) * MICRO_PER_CENT_BI;
}
 ...
 */
export function levelUpBuyTotalMicroScwBalanceRequired(makerAmountMicro: bigint): bigint {
