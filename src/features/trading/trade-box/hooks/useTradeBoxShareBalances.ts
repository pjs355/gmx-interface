/**
 * Cross-venue share balances for sell UI, My Positions, and SOR sell routing.
 *
 * Aggregates LevelUp, Poly, Predict, Limitless, DFlow holdings for the active
 * umbrella/monitor mapping. Feeds max sell shares, venue breakdown strip, and
 * All Markets sell bid selection.
 */
import { useEffect, useMemo, useState } from "react";
import { usePredictionData } from "@/context/PredictionDataContext";
import { usePrivy } from "@privy-io/react-auth";
import { useAccountLevelUpPositions, useVenueAddressChainMap } from "@/context/AccountDataContext";
...
} from "@/features/trading/venues/limitless/trade/limitlessTradeBoxMatch";
import {
	inferLimitlessCatalogYesColumn,
	inferLimitlessYesColumnFromOutcomeTitle,
} from "@/features/trading/venues/limitless/trade/limitlessCatalogTokenPair";
import { usePolymarketPositions } from "@/features/trading/venues/polymarket/portfolio/usePolymarketPositions";
import { usePredictPositions } from "@/features/trading/venues/predict/portfolio/usePredictPositions";
...
} from "@/features/trading/venues/predict/trade/predictTradeBoxMatch";
...
import { inferPredictSideFromMarketDetail } from "@/features/trading/venues/predict/trade/predictPositionSide";
import {
...
import type { LimitlessInferenceWire } from "@/features/trading/venues/limitless/trade/limitlessCatalogTokenPair";
import {
	type VenuePosition,
	type VenueId,
...
} from "@/features/trading/venues/polymarket/trade/polyPositionSide";
import {
...
import { resolveLimitlessInferenceWireForUmbrella } from "@/features/markets/odds-monitor/mergeMonitorLimitlessFromUmbrella";
...
function shareBalanceLoadingWaitsForVenue(
	...
	switch (venue) {
		...
		case "dflow":
			return Boolean(
				matchedMonitor.dflow?.eventTicker?.trim() || matchedMonitor.dflow?.eventTicker?.trim(),
			);
		case "limitless": {
			const lx = matchedMonitor.limitless;
			return Boolean(
				lx?.slug?.trim() || (Boolean(lx?.tokenIdA?.trim()) && Boolean(lx?.tokenIdB?.trim())),
			);
		}
		default:
			return true;
 ...
 *   - predictfun → {@link matchVenuePositionToUmbrella} (same as `useUmbrellaPositions`), then title fallback if lookup misses
...
function umbrellaForPosition(
	...
	predictUmbrellaLookup: PredictUmbrellaLookup | null,
	predictTitleHint: string | null | undefined,
): Umbrella | null {
	...
	if (pos.venue === "predictfun") {
		const fromPredict = matchVenuePositionToUmbrella(
			pos,
			"predictfun",
			condLookup,
			umbrellas,
			predictUmbrellaLookup,
			predictTitleHint ?? null,
			dflowMintLookup,
	...
	if (pos.venue === "dflow") {
		...
		if (et) {
			const byEt = lookupUmbrellaByDflowEventTicker(et, dflowEventTickerLookup, umbrellas);
			if (byEt) return byEt;
		}
		if (pos.tokenId?.trim()) {
...
function positionMatchesMarket(pos: VenuePosition, market: MarketRef): boolean {
	...
	/**
	 * Predict: same contract as Positions — `umbrellaForPosition` already used
	 * {@link matchVenuePositionToUmbrella} + monitor token/market id; do not re-gate on child
	 * `market` title vs `pos.marketTitle` (API strings diverge from catalog `displayName`).
	 */
	if (pos.venue === "predictfun") return true;
...
/**
 * Map held Predict token → LevelUp yes/no using the odds-monitor row (same A/B + tokenIdA/B
 * wiring as `PredictionMarketTradeBox` `predictTokenIdForPosition`, inverted).
 */
function predictPositionToYesNoFromMonitor(
	p: VenuePosition,
	...
	if (!pf || !matched) return null;
	const pt = normalizePredictTokenId(p.tokenId ?? "");
	if (!pt) return null;
	const yesTokRaw = (() => {
		const side = pandaOutcomeSide(matched, "yes", yesTeamLabel, noTeamLabel);
		return side === "A" ? pf.tokenIdA : (pf.tokenIdB ?? pf.tokenIdA);
	})();
	const noTokRaw = (() => {
		const side = pandaOutcomeSide(matched, "no", yesTeamLabel, noTeamLabel);
		return side === "A" ? pf.tokenIdA : (pf.tokenIdB ?? pf.tokenIdA);
	})();
	if (!yesTokRaw?.trim() || !noTokRaw?.trim()) return null;
	const yesTok = normalizePredictTokenId(yesTokRaw);
	const noTok = normalizePredictTokenId(noTokRaw);
	if (!yesTok || !noTok) return null;
	if (pt === yesTok) return "yes";
	if (pt === noTok) return "no";
	return null;
}
...
function polymarketPositionToYesNo(
	...
	const pageCid = pageMatchedMonitor ? String(pageMatchedMonitor.polyConditionId ?? "").trim() : "";
	const cidKey = polymarketConditionLookupKey(cid);
	const pageCidKey = polymarketConditionLookupKey(pageCid);
	const matchedFromPage =
		pageMatchedMonitor && cidKey && pageCidKey && cidKey === pageCidKey ? pageMatchedMonitor : null;
	const matched =
...
function venuePositionToYesNo(
	...
	if (p.venue === "limitless") {
		...
		return inferLimitlessYesColumnFromOutcomeTitle(p.outcome, p.marketTitle) ? "yes" : "no";
	}
	if (p.venue === "predictfun") {
		const positionDetail =
			p.numericMarketId != null && Number.isFinite(p.numericMarketId)
				...
				: undefined;
		const fromDetail = inferPredictSideFromMarketDetail(positionDetail ?? null, p.tokenId);
		if (fromDetail) return fromDetail.side === "Yes" ? "yes" : "no";
		const fromMonitor = predictPositionToYesNoFromMonitor(
			p,
			pageMatchedMonitor,
			yesTeamLabel,
			noTeamLabel,
...
function buildOutcomeVenueBreakdownRows(
	...
	for (const key of venueOrder) {
		...
		if (sh != null && sh > 0) {
			const display = key === "levelup" ? VENUE_SUFFIX.levelup : VENUE_SUFFIX[key as VenueId];
...
export function useTradeBoxShareBalances(opts: {
	...
	const limitlessPortfolioEnabled = Boolean(authenticated) && Boolean(limitlessMakerBase?.trim());
	const limitlessVenueQ = useLimitlessVenuePositions(venueReady && limitlessPortfolioEnabled);
	...
	const polyQ = usePolymarketPositions(venueEnabled && polymarketSafe ? polymarketSafe : null);
	const predictQ = usePredictPositions(venueEnabled && predictWallet ? predictWallet : null);
	...
	const relevantVenuePositions = useMemo(() => {
		...
		for (const p of [
			...
			if (p.venue === "polymarket" && polyMonitorKey) {
				const pcKey = polymarketConditionLookupKey(String(p.conditionId ?? "").trim());
			...
			if (!keep) {
				...
				if (
					!positionMatchesMarketOrSiblings(p, market, siblingConditionIds, predictPositionMatchCtx)
				)
					continue;
				keep = true;
			}
	...
	const levelBalances = useMemo(() => {
		...
		return {
			yes: readSideShares(String(marketId), "yes"),
			no: readSideShares(String(marketId), "no"),
		};
	}, [market?._id, market?.questionId, readSideShares, levelUpRows, getMarketBalance]);

	const waitPoly = shareBalanceLoadingWaitsForVenue(pageMatchedMonitor, "polymarket");
	const waitPredict = shareBalanceLoadingWaitsForVenue(pageMatchedMonitor, "predictfun");
	const waitDflow = shareBalanceLoadingWaitsForVenue(pageMatchedMonitor, "dflow");
	const waitLimitless = shareBalanceLoadingWaitsForVenue(pageMatchedMonitor, "limitless");
	...
	const buyLines = useMemo(() => lines.map((l) => ({ ...l, venueSuffix: null })), [lines]);
