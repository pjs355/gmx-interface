/**
 * Guards venue tab selection when smart routing is unavailable or trade is in flight.
 *
 * Auto-switches off invalid tabs (e.g. no pandascore link) and exposes sell-input
 * rules (`shareAmountRequiresWholeContracts`, dollar prefix) from venue config.
 */
...
export function useTradeBoxVenueTabGuard(opts: {
	...
	const venueDropdownOptions = useMemo(() => {
		...
		const venues = opts.matchedVenues ? all.filter((v) => opts.matchedVenues!.has(v.value)) : all;
