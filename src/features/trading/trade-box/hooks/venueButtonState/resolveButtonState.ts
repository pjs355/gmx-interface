...
export function resolveButtonState(ctx: ButtonStateResolveContext): ButtonStateResult {
	...
	const checkInputMinForButtonLabel = (tradingVenue: string, matched?: Iterable<string> | null) =>
		checkRawInputAgainstVenueMinimum({
			tradingVenue,
	...
	const belowMinButton = (
		check: Extract<ReturnType<typeof checkInputMinForButtonLabel>, { below: true }>,
	): ButtonStateResult => ({
		text: check.message,
		disabled: true,
		onClick: () => {},
	...
	if (state.tradingVenue === "polymarket") {
		...
		if (pt.loading && !pt.ready) {
			return {
				text: globalSetupInProgress ? SETUP_IN_PROGRESS_LABEL : VENUE_LOADING_LABEL,
				disabled: true,
				onClick: () => {},
			};
		}
		...
		const sorBuy = sorUnifiedPrimary(state.side, sorState, buttonText, animatedDots, {
			sellAmountStr: state.amount,
			maxSellShares: scopedSellSharesTotal(),
			globalSetupInProgress,
		});
	...
	if (state.tradingVenue === "limitless") {
		...
		if (state.orderType === "market" && state.selectedPosition) {
			const liqPosition = orderbookWalkPosition ?? state.selectedPosition ?? "yes";
			const liquidityInfo = marketOrderHandler.getAvailableLiquidity(liqPosition, state.side);
			if (state.side === "sell") {
				...
				if (
					...
					requested > 0 &&
					requested > liquidityInfo.maxSharesAvailable + SHARE_SELL_COMPARE_EPS
				) {
					return {
		...
		const sorLx = sorUnifiedPrimary(state.side, sorState, buttonText, animatedDots, {
			sellAmountStr: state.amount,
			maxSellShares: scopedSellSharesTotal(),
			globalSetupInProgress,
		});
	...
	if (state.tradingVenue === "dflow") {
		...
		const sorDf = sorUnifiedPrimary(state.side, sorState, buttonText, animatedDots, {
			sellAmountStr: state.amount,
			maxSellShares: scopedSellSharesTotal(),
			globalSetupInProgress,
		});
	...
	if (state.tradingVenue === "predictfun") {
		...
		const skipPredictBootstrapGate = state.side === "sell" || pt.approvalsOk === true;
		...
		const venueAutoSetupInFlight = Boolean(predictTrading?.loading) && !skipPredictBootstrapGate;
		const sorPf = sorUnifiedPrimary(state.side, sorState, buttonText, animatedDots, {
			venueAutoSetupInFlight,
			sellAmountStr: state.amount,
			maxSellShares: scopedSellSharesTotal(),
			globalSetupInProgress,
	...
	if (state.orderType === "market" && state.selectedPosition) {
		if (state.tradingVenue === "all" && state.side === "buy" && sorState?.route) {
	...
	const fallbackButtonText = buildTradeActionButtonText(state.side, state.selectedPosition, market);

	if (state.tradingVenue === "levelup") {
		const sorLu = sorUnifiedPrimary(state.side, sorState, fallbackButtonText, animatedDots, {
			sellAmountStr: state.amount,
			maxSellShares: scopedSellSharesTotal(),
			globalSetupInProgress,
		});
