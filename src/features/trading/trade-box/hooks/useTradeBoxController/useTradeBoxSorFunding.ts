/**
 * SOR prefund / chain balance inputs before route execution.
 *
 * Builds per-chain USDC balances and venue position keys for funding checks.
...
export function useTradeBoxSorFunding(
	...
	const sorWalletBalances: ChainBalance[] = useMemo(() => {
		if (!isVacmReady({ venueAddressChainMap, walletGate }) || venueAddressChainMap == null) {
			return [];
		}
	...
	const sorVenuePositions: VenuePositionEntry[] = useMemo(() => {
		...
		const sideMap = state.selectedPosition === "yes" ? byOutcome.yes : byOutcome.no;
