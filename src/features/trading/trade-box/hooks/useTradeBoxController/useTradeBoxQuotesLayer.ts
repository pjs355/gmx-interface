/**
 * Debounced SOR route preview + execution route fetching for the trade box.
 *
 * Combines display vs execution route queries, venue previews, and amount derivation.
 * Sub-hook of `useTradeBoxController`.
 */
...
import { parseLimitPriceCents } from "@/features/trading/sor";
import type { UseSorRouteResult } from "@/features/trading/sor/core/useSorRoute";
import {
...
import type { MarketOrderBookPreview, TradeQuote } from "@/features/trading/trade-preview/types";
import { useTradePreview } from "@/features/trading/trade-preview/useTradePreview";
...
export function useTradeBoxQuotesLayer(
	...
	const sorAmountUsd = useMemo(
		...
		[state.amount, state.side, state.orderType, sorLimitPriceCents, maxScopedSellShares],
	);
	...
	const sorAmountMeetsFloor = useMemo(() => {
		...
		return Number.isFinite(sharesIn) && sharesIn + 1e-9 >= SOR_MIN_MARKET_SELL_SHARES;
	}, [sorAmountUsd, state.orderType, state.side, state.amount]);

	const debouncedSorRoutePreviewAllowed = useDebouncedValue(sorAmountMeetsFloor, 350);

	const smartRoutingMarketKey = useMemo(() => getMarketId(market).trim(), [market]);

	const sorQuestionId = useMemo(() => getMarketId(market) || undefined, [market]);
	...
	const tradeBoxQuotes = useTradeBoxQuotes({
		...
		sorRoute: {
			...
			Number.isFinite(limitlessMakerCashForSor)
				...
				: {}),
			...(state.side === "buy" ? { limitlessFeeRateBps: LIMITLESS_DEFAULT_FEE_RATE_BPS } : {}),
	...
	const executableErrorCode =
		state.tradingVenue === "all" ? sorRoute.displayErrorCode : sorRoute.executionErrorCode;

	const tradeQuoteBase = useTradePreview({
