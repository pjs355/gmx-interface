 ...
 */
import type { LifiAllowanceHint, LifiQuoteStep, LifiSolanaDelegateHint } from "@/types/trading";
...
function buildEvmAllowanceHint(step: Record<string, unknown>): LifiAllowanceHint | undefined {
	...
	const spender = typeof est.approvalAddress === "string" ? est.approvalAddress.trim() : "";
	if (!spender) return undefined;
	const fromTok = asRecord(action.fromToken);
	const tokenAddress = fromTok && typeof fromTok.address === "string" ? fromTok.address.trim() : "";
	const fromAmount = typeof action.fromAmount === "string" ? action.fromAmount.trim() : "";
...
function buildSolanaDelegateHint(
	...
	const action = asRecord(step.action);
	if (!est || !action) return undefined;
	const delegate = typeof est.approvalAddress === "string" ? est.approvalAddress.trim() : "";
	if (!delegate || !SOLANA_BASE58_RE.test(delegate)) return undefined;
	const fromTok = asRecord(action.fromToken);
	const mint = fromTok && typeof fromTok.address === "string" ? fromTok.address.trim() : "";
	const fromAmount = typeof action.fromAmount === "string" ? action.fromAmount.trim() : "";
...
function inferExecutableStepKind(step: LifiQuoteStep): "evm" | "solana" {
	...
	const b64 =
		typeof step.transactionDataBase64 === "string" ? step.transactionDataBase64.trim() : "";
