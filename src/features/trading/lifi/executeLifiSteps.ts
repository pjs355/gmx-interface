...
import { BSC_RPC_URL, DEFAULT_RPC_URL, createSolanaConnectionForWalletSend } from "@/config/rpc";
import {
...
} from "@/features/trading/lifi/lifiTransferFromFailed";
import { mergeLifiStepsWithRawAllowanceMetadata } from "@/features/trading/lifi/lifiExecutableStepMetadata";
import { ensureSolanaSplDelegateAllowanceIfNeeded } from "@/features/trading/lifi/ensureSolanaLifiSplDelegate";
...
async function ensureAllowance(
	...
	if (receipt.status !== "success") {
		throw new Error(`ERC-20 approve reverted on chain ${hint.chainId} (step ${stepLabel})`);
	}
...
function relayTransactionFromTr(tr: LifiTransactionRequest): Transaction {
	...
	const value = tr.value && tr.value !== "0x0" && tr.value !== "0" ? tr.value : "0";
	return {
		to: tr.to,
		data,
		value,
...
export async function executeLifiSteps(
	...
	for (let i = 0; i < ordered.length; i++) {
		...
		if (chainId !== SOLANA_LIFI_CHAIN_ID && (!tr.to || !ETH_ADDRESS_RE.test(tr.to))) {
			throw new Error(`LI.FI step ${i} has invalid 'to' address: ${tr.to ?? "(missing)"}`);
		}
		...
		if (evmAllowanceHint) {
			...
			try {
				const approveHash = await ensureAllowance(owner, evmAllowanceHint, signer, String(i));
