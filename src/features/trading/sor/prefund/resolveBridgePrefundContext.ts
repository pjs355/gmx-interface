...
import { maskFundingAddress } from "@/features/trading/sor/execute/bridgeHelpers";
...
import { chainsForBridgeCorridor } from "@/features/trading/sor/prefund/fundingStableBalanceChains";
import {
	planLimitlessScwSweepMicros,
	recappedSweepForSend,
} from "@/features/trading/sor/prefund/limitlessPrefundSweep";
import {
...
} from "@/features/trading/sor/prefund/prefundPlan";
import {
...
} from "@/features/trading/sor/prefund/venuePrefundAdjustments";
...
export async function resolveBridgePrefundContext(input: {
	...
	const balancesHuman = await readFundingStableBalancesForChains(fundingAddresses, prefundChains);
	const levelUpBaseBridgedPrefund = isLevelUpBaseBridgedPrefund(leg, bridge);
	const { balancesForShortfall, scwPendingUsdcHuman } = await adjustBalancesForLevelUpPendingUsdc({
		leg,
		bridge,
		balancesHuman,
	...
	if (limitlessBaseDest && bridgeShortfallUsd > PREFUND_SHORTFALL_COVERED_EPS_USD) {
		...
		let sweepPlan = planLimitlessScwSweepMicros(bridgeShortfallUsd, b1, MIN_PREFUND_CHUNK_USD);
	...
	const prefundLogBase = {
		...
		scwPendingUsdcApprox: levelUpBaseBridgedPrefund ? Number(scwPendingUsdcHuman.toFixed(6)) : null,
