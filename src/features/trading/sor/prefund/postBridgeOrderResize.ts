 ...
 */
const POST_BRIDGE_RESIZE_DUST_USD = 0.005;

type PostBridgeClampInput = {
	/**
 ...
 */
export function clampMarketBuyAmountToWallet(args: PostBridgeClampInput): PostBridgeClampResult {
	...
	const planned = Number.isFinite(args.plannedExecutionUsd)
		...
		: 0;
	const minOrder = Number.isFinite(args.minOrderUsd) ? Math.max(0, args.minOrderUsd) : 0;
