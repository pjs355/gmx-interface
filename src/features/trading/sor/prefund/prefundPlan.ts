...
import { floorFloatToDecimalString } from "@/features/trading/lifi/prefundFromAmountHuman";
...
function splitBasePrefundAmount(need: number, pool: MutableBasePool): PrefundStep[] {
...
export function formatPrefundBalanceBreakdown(
	...
	const parts = ALL_SOURCE_CHAINS.map((c) => {
		...
		const tag = c === toChain ? " (venue — counts first toward prefund; reduces LI.FI pull)" : "";
	...
	const lxPart =
		lx > 0 || opts?.limitlessBaseDest ? ` | Limitless maker (Base) $${lx.toFixed(2)}${lxNote}` : "";
