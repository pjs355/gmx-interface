...
import { BSC_RPC_URL, createSolanaConnectionForJsonRpcReads, DEFAULT_RPC_URL } from "@/config/rpc";
import { POLYGON_PUSD, POLYGON_USDC_E } from "@/features/trading/venues/polymarket/trade/constants";
import {
...
export type CollateralChainKey = "base" | "polygon" | "bnb" | "solana" | "limitlessMakerBase";
	...
	return {
		...
		bnbAddr:
			addrs.embeddedEoa && /^0x[a-fA-F0-9]{40}$/i.test(addrs.embeddedEoa)
				...
				: undefined,
		solAddr:
			addrs.solanaAddress && addrs.solanaAddress.length >= 32 && addrs.solanaAddress.length <= 44
				? addrs.solanaAddress
				: undefined,
		limitlessAddr:
...
export async function readBaseEmbeddedUsdcBalanceRaw(
	...
	const addr =
		embeddedEoa && /^0x[a-fA-F0-9]{40}$/i.test(embeddedEoa) ? (embeddedEoa as Address) : undefined;
	if (!addr) return 0n;
	return basePublic.readContract({
		address: getUSDCAddress() as Address,
		abi: erc20Abi,
