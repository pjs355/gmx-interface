import { formatErrorForUser, userMessage, LIFI_BRIDGE_FAILED, LIFI_NO_BRIDGE_DATA } from "@/errors";
...
import { buildPrefundSteps } from "@/features/trading/sor/prefund/prefundPlan";
...
} from "@/features/trading/sor/prefund/resolveBridgePrefundContext";
import { runLimitlessScwToMakerSweep } from "@/features/trading/sor/prefund/runLimitlessScwToMakerSweep";
import { runPrefundLifiSteps } from "@/features/trading/sor/prefund/runPrefundLifiSteps";
...
export async function runBridgePrefund(input: SorBridgeExecuteInput): Promise<SorBridgeResult> {
	const {
		leg,
		fundingAddresses,
		opts,
	...
	try {
		...
		if (plannedSweepMicros > 0n) {
			console.debug("[SOR][prefund] parallel settle: Base SCW → maker receipt + LI.FI terminal", {
				venue: leg.venue,
