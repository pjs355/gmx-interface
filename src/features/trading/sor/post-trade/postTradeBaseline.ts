...
import type { RouteExecution, RouteLeg, RoutePlan, SorChain, SorVenue } from "../core/sor-types";
...
import { normalizePolymarketPositionTokenId } from "@/features/trading/venues/polymarket/portfolio/polymarketPositionsRefetchMerge";
...
export function chainToCollateralKey(chain: SorChain, venue: SorVenue): CollateralChainKey {
...
export function shareIdentityForRouteLeg(
	...
	switch (leg.venue) {
		case "polymarket": {
			const raw =
				leg.outcome === "A" ? leg.venueMarketIds.polyTokenIdA : leg.venueMarketIds.polyTokenIdB;
			const tok = normalizePolymarketPositionTokenId(raw);
			return tok ? `polymarket:${tok}` : null;
		}
		case "predictfun": {
			const tokRaw = leg.outcome === "A" ? ctx?.predictFun?.tokenIdA : ctx?.predictFun?.tokenIdB;
		...
		case "limitless": {
			const slug =
				leg.outcome === "A"
					? String(leg.venueMarketIds.limitlessOrderbookSlugA ?? "")
							.trim()
							.toLowerCase()
					: String(leg.venueMarketIds.limitlessOrderbookSlugB ?? "")
							.trim()
							.toLowerCase();
			if (slug) return `limitless:leg:${slug}`;
			const raw =
...
function lookupCachedShares(
	...
	switch (venue) {
		case "polymarket": {
			...
			if (!safe) return 0;
			return findShares(queryClient.getQueryData<VenuePosition[]>(["polymarket-positions", safe]));
		}
		case "predictfun": {
		...
		case "limitless": {
			return findShares(
				queryClient.getQueryData<VenuePosition[]>(limitlessQueryKeys.positionsVenue),
			);
		}
		default:
			return 0;
 ...
 */
export function capturePostTradeBaseline(input: PostTradeBaselineInput): PostTradeBaseline {
	...
	for (const leg of input.route.legs) {
		...
		shares.set(
			identity,
			lookupCachedShares(input.queryClient, leg.venue, identity, input.addresses),
		);
	}
...
export function computeExpectedDeltas(
	...
	for (let i = 0; i < execution.legs.length; i++) {
		...
		const next = route.side === "buy" ? cur + filled : Math.max(0, cur - filled);
