...
import { pollWithMaxAttempts } from "@/features/trading/sor/post-trade/pollAccountRefresh";
import {
...
} from "@/features/trading/sor/post-trade/performPostTradeDataRefresh";
...
const PostTradeAccountSyncContext = createContext<PostTradeAccountSyncApi | null>(null);
...
export function PostTradeAccountSyncProvider({ children }: { children: ReactNode }) {
		...
		void (async () => {
			...
			try {
				...
				const tryMarkSynced = (): boolean => {
					...
					const stillWaitingOnVenueShares = unresolved.some((t) => t.kind === "shares");
					if (
				...
				if (sessionRef.current === session) {
					await runPostTradeExitBurst(input.queryClient, input, account, initialPending);
	...
	const startBlindBalanceRefresh = useCallback((breq: BlindPostTradeBalanceRefreshRequest) => {
		const session = ++sessionRef.current;
		...
		void (async () => {
			try {
				...
				for (let i = 0; i < iterations; i++) {
					if (sessionRef.current !== session) return;
					await performPostTradeDataRefreshPass(breq.queryClient, reqSlice, account, opts);
				...
				if (sessionRef.current === session) {
					await runPostTradeExitBurst(breq.queryClient, reqSlice, account, synthetic);
				}
			} finally {
				clearTimeout(wallClockTimer);
				if (sessionRef.current === session) {
			...
			void (async () => {
				try {
					for (let attempt = 0; attempt < MAX_REFETCH_ATTEMPTS; attempt++) {
						...
						await refetchCollateralCachesForClaim(req.queryClient, req.refetchCollateral);

						const obs = readTotalCashHumanFromQueryClient(req.queryClient);
						if (
							obs != null &&
							valueDiverged(obs, req.baselineTotalCash, CASH_CONVERGENCE_TOL_USD)
						) {
							break;
						}
...
export { readTotalCashHumanFromQueryClient } from "@/features/trading/sor/post-trade/performPostTradeDataRefresh";
export type { AccountReconcileSnapshot } from "@/features/trading/sor/post-trade/postTradeReconcile";
