...
import type { AccountVenueKey, AccountRemoteVenueKey } from "@/context/AccountDataContext";
import type { VenuePosition } from "@/types/trading/venuePosition";
import { COLLATERAL_TOKENS_QUERY_KEY } from "@/context/CollateralTokenContext";
import type { CollateralChainKey } from "@/features/trading/sor/prefund/fundingStableBalances";
...
import { getCachedDflowPositions } from "@/features/trading/venues/dflow/portfolio/dflowPositionsQueryCache";
...
} from "@/features/trading/sor/post-trade/postTradeVenueRefresh";
...
export function readTotalCashHumanFromQueryClient(queryClient: QueryClient): number | null {
	const all = queryClient.getQueriesData<FundingStableBalancesHuman>({
		queryKey: [COLLATERAL_TOKENS_QUERY_KEY],
	});
	for (const [, data] of all) {
		if (!data) continue;
		return data.base + data.polygon + data.bnb + data.solana + (data.limitlessMakerBase ?? 0);
	}
	return null;
}
...
export function pendingHasPolymarketShares(pending: PostTradePendingTarget[]): boolean {
...
export function readVenueShares(
	...
	switch (venue) {
		case "polymarket": {
			...
			if (!safe) return null;
			const rows = queryClient.getQueryData<VenuePosition[]>(["polymarket-positions", safe]);
			if (rows === undefined) return null;
			return findShares(rows);
		}
		case "predictfun": {
			const wallet = (addresses.predictWallet ?? "").trim().toLowerCase() || null;
			if (!wallet) return null;
			const rows = queryClient.getQueryData<VenuePosition[]>(["predict-positions", wallet]);
			if (rows === undefined) return null;
			const n = findShares(rows);
			if (n > 0) return n;
			const body = identity.startsWith("predictfun:") ? identity.slice("predictfun:".length) : "";
		...
		case "limitless": {
			const rows = queryClient.getQueryData<VenuePosition[]>(limitlessQueryKeys.positionsVenue);
			if (rows === undefined) return null;
			return findShares(rows);
		}
		default:
...
function readCashForChain(queryClient: QueryClient, chain: CollateralChainKey): number | null {
	const all = queryClient.getQueriesData<FundingStableBalancesHuman>({
		queryKey: [COLLATERAL_TOKENS_QUERY_KEY],
	});
	for (const [, data] of all) {
...
export function buildWatchTargets(
	...
	const deltas = computeExpectedDeltas(route, execution, baseline, shareIdentityCtx);
	...
	if (deltas.levelUp && baseline.levelUp) {
		...
		if (routeId && baselineId && routeId !== baselineId && import.meta.env.DEV) {
			console.warn(
		...
		const baselineLevelUp =
			deltas.levelUp.side === "yes" ? baseline.levelUp.yes : baseline.levelUp.no;
...
export function valueDiverged(observed: number | null, baseline: number, tol: number): boolean {
...
function venueToAccountVenue(venue: VenuePosition["venue"]): AccountVenueKey | null {
	switch (venue) {
		case "polymarket":
			return "polymarket";
		case "predictfun":
...
export function buildSyntheticBlindPending(
	...
	const out: PostTradePendingTarget[] = [];
	const map: Record<AccountRemoteVenueKey, VenuePosition["venue"]> = {
...
export async function performPostTradeDataRefreshPass(
	...
	const venuesToRefresh = new Set(opts.venueShareKeys.map((k) => accountVenueKeyToRefreshKey(k)));
	if (opts.levelUpRpc) {
	...
	if (opts.cash) {
		pushTask("postTradeVenueRefresh.cash", runPostTradeCashRefresh(queryClient, account));
	}
