...
function writeSorRouteCache(key: string | null, entry: CachedRouteEntry): void {
	if (!key) return;
...
export function useSorRoute(input: UseSorRouteInput): UseSorRouteResult {
	...
	const initialExecutionCacheEntry = readSorRouteCache(initialExecutionCacheKey);

	const [displayRoute, setDisplayRoute] = useState<RoutePlan | null>(
		initialDisplayCacheEntry?.route ?? null,
	);
	const [displayRouteSourceQuestionId, setDisplayRouteSourceQuestionId] = useState<string | null>(
	...
	const [executionRouteSourceQuestionId, setExecutionRouteSourceQuestionId] = useState<
		string | null
	>(
		initialExecutionCacheEntry?.questionId ??
			(questionId && initialExecutionCacheEntry?.route ? questionId : null),
	);
	const [venuePreviews, setVenuePreviews] = useState<VenueRoutePreview[] | null>(
	...
	const buildRequest = useCallback(
		(channel: ChannelKind): RouteRequest => {
			...
			const pondOnThisChannel =
				...
				((channel === "display" && !targetVenue) || (channel === "execution" && !!targetVenue));
			return {
	...
	const doFetchChannel = useCallback(
		async (channel: ChannelKind) => {
			...
			const applySuccess = (r: Extract<SorRouteResult, { success: true }>) => {
				...
				const cacheVenuePreviews = channel === "display" ? (r.venuePreviews ?? null) : null;
			...
			const surfaceFailure = (opts: {
				...
				const silentAuthFailure = opts.code == null && isAuthShapedSorError(opts.message);
	...
	useEffect(() => {
		...
		const fireImmediately = hardReset || outcomeOnlyFlip || targetVenueChanged;
		if (fireImmediately) {
