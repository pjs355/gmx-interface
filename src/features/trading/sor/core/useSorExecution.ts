...
import { validateLegMinimum } from "../route/sorPreflight";
import type { RoutePlan, RouteLeg, RouteExecution, SorVenue } from "./sor-types";
import { groupBridgeLegsByCorridor } from "../prefund/sorBridgeGroups";
import { LEG_OR_BRIDGE_TIMEOUT_MS } from "@/features/trading/sor/prefund/sorBridgeWallTimeBudget";
...
function buildLocalExecution(
	...
	const legs = route.legs.map((leg) => {
		...
		return {
			...
			...(result?.initializedMarket === true ? { initializedMarket: true as const } : {}),
			...(result?.dflowPartialFill === true ? { dflowPartialFill: true as const } : {}),
			error: errorForLeg,
			updatedAt: Date.now(),
		};
	});
...
export function useSorExecution(input: UseSorExecutionInput): UseSorExecutionResult {
	...
	const [prefundLegProgress, setPrefundLegProgress] = useState<SorPrefundLegProgress | null>(null);
	...
	const trySyncBackend = useCallback(async (fn: () => Promise<void>) => {
		try {
			await fn();
		} catch (err) {
	...
	const execute = useCallback(
		async (route: RoutePlan): Promise<RouteExecution | null> => {
			...
			const legPlanSummary = route.legs.map((l) => ({
				...
				bridgeCorridor: l.bridge != null ? `${l.bridge.fromChain}→${l.bridge.toChain}` : null,
				executionAmountUsd: l.executionAmountUsd,
			}));
			console.debug("[SOR] leg plan (LiFi prefund vs immediate venue)", legPlanSummary);

			if (route.side === "buy") {
				const dflowNoBridge = route.legs.filter((l) => l.venue === "dflow" && !l.bridge);
			...
			try {
				...
				const immediateLegs = isSell ? eligibleLegs : eligibleLegs.filter((l) => !l.bridge);
				const bridgeLegs = isSell ? [] : eligibleLegs.filter((l) => !!l.bridge);
				...
				const bridgeGroups = groupBridgeLegsByCorridor(bridgeLegs, isSell ? "sell" : "buy");
				for (const group of bridgeGroups) {
					...
					try {
						bridgeResult = await withTimeout(
							executeBridge(group.representativeLeg, {
								...
								budgetUsdOverride: group.totalAmountUsd + group.groupBridgeCostUsd,
								strictLifiDestMinAtSendCap: group.legs.some((l) => l.venue === "levelup"),
								onPrefundProgress: (p) => {
									if (mountedRef.current) {
					...
					if (!bridgeResult.success) {
						...
						for (const leg of group.legs) {
							...
							trySyncBackend(() =>
								apiClient
									.updateLeg(route.routeId, leg.venue as SorVenue, "failed", {
										error: bridgeErr,
