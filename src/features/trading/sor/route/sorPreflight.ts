import type { RouteLeg, SorVenue } from "../core/sor-types";
...
function venuesForMinimumCheck(
	...
	if (tradingVenue === "all") {
		const fromMatch = matchedVenues ? [...matchedVenues].filter(isSorVenue) : [];
...
export function aggregateMinThresholds(venues: readonly SorVenue[]): AggregateVenueMinimums {
	...
	return {
		maxMinMarketBuyUsd: Math.max(...venues.map((v) => VENUE_MINIMUMS[v].minMarketBuyUsd)),
		maxMinMarketSellShares: Math.max(...venues.map((v) => VENUE_MINIMUMS[v].minMarketSellShares)),
		maxMinLimitShares: Math.max(...venues.map((v) => VENUE_MINIMUMS[v].minLimitShares)),
 ...
 */
export type BelowMinReason = "marketBuy" | "marketSell" | "limitOrder" | "limitShares";
...
export function checkRawInputAgainstVenueMinimum(args: {
	...
	const venues = venuesForMinimumCheck(args.tradingVenue, args.matchedVenues ?? null);
	...
	if (args.orderType === "limit") {
		...
		if (
			...
			const floor = Math.max(agg.maxMinLimitBuyNotionalUsd, SOR_MIN_LIMIT_ORDER_USD);
	...
	const sellFloor = Math.max(agg.maxMinMarketSellShares, SOR_MIN_MARKET_SELL_SHARES);
	if (n + EPS < sellFloor) {
		return {
			below: true,
			reason: "marketSell",
