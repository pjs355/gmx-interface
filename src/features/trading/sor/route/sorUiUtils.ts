import type { OddsDisplayStyle } from "@/features/odds-display/oddsDisplayFormat";
import { formatOddsPrice } from "@/features/odds-display/oddsDisplayFormat";
import type { RoutePlan, SorChain } from "../core/sor-types";
 ...
 */
export function formatSorLegAvgForDisplay(p: number, style: OddsDisplayStyle): string {
 ...
 */
export function derivedBridgeUsdForDisplay(route: RoutePlan): {
	displayUsd: number;
	legSumUsd: number;
} {
	if (route.side === "sell") {
...
export function getSorLifiTransferFeeRowState(route: RoutePlan): {
	...
	const show = displayUsd > 0 || legSumUsd > 0 || apiReports || hasLegBridge;
...
export function getSorBuyCashShortfall(
	...
	if (typeof needed !== "number" || !Number.isFinite(needed) || needed <= totalAvailableCash)
		return null;
