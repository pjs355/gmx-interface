...
describe("post-trade share direction vs baseline", () => {
	...
	it("pendingWatchTargetsResolved: venue null when positions query missing", () => {
		...
		expect(pendingWatchTargetsResolved(pending, qc, emptyAddresses, () => 0)).toBe(false);
	});
	...
	it("pendingWatchTargetsResolved: venue hydrated and directional move", () => {
		...
		expect(pendingWatchTargetsResolved(pending, qc, emptyAddresses, () => 0)).toBe(true);
	});
	...
	it("filterUnresolvedPending keeps venue row until query hydrates", () => {
		...
		expect(filterUnresolvedPending(pending, qc, emptyAddresses, () => 0)).toHaveLength(1);
		qc.setQueryData(limitlessQueryKeys.positionsVenue, [limitlessRow(3)]);
		expect(filterUnresolvedPending(pending, qc, emptyAddresses, () => 0)).toHaveLength(0);
	});
	...
	it("LevelUp empty marketId uses strict direction (no tolerance shortcut)", () => {
		...
		expect(pendingWatchTargetsResolved(pending, qc, emptyAddresses, () => 3)).toBe(true);
		expect(pendingWatchTargetsResolved(pending, qc, emptyAddresses, () => 5)).toBe(false);
	});
	...
	it("invalidates polymarket-positions and refetches venue positions", async () => {
		...
		expect(
			invalidateSpy.mock.calls.some(
				(c) => Array.isArray(c[0]?.queryKey) && c[0].queryKey[0] === "polymarket-positions",
			),
		).toBe(true);
		...
		expect(
			refetchSpy.mock.calls.some(
				(c) => Array.isArray(c[0]?.queryKey) && c[0].queryKey[0] === "predict-market",
			),
		).toBe(false);
	});
