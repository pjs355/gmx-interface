 ...
 */
export function stripUmbrellaDisplayPrefix(name: string | undefined | null): string {
	if (!name) return "";
	let s = name.trim();
 ...
 */
export function shortPredictFunMarketTitleForPortfolio(raw: string | undefined | null): string {
	const t = (raw ?? "").trim();
	if (!t) return "";
	/** e.g. "Will Team Nemesis win the Heroic vs. Team Nemesis CS2 match?" → match line */
	const predictWinner = t.match(/\bwin\s+the\s+(.+?)\s+(?:CS2\s+)?match\??\s*$/i);
	if (predictWinner?.[1]) {
