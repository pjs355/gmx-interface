...
/**
 * venue-prices WS → `MatchedMarket` book fields. When adding a venue, extend this map and
 * `venueWireNameToKey` to match predictions-api; then add a display adapter (see
 * `features/markets/pricing/venuePriceAdapters/index.ts`).
 */
const VENUE_PRICE_FIELDS: Record<VenueKey, [keyof MatchedMarket, keyof MatchedMarket][]> = {
	...
	/** Legacy venue id on wire; same `dflowPrice*` storage. */
...
export function useOddsMonitorWebSocket(
		...
		const run = (async () => {
			try {
				...
				if (verboseVenueMappings) {
					console.group(
						`[venue-monitor] 🔍 Umbrella Matched-Markets Breakdown (${items.length} items)`,
					);
		...
		const connect = () => {
			...
			try {
				...
				ws.onopen = () => {
					...
					const want = new Set(
						activePandaMatchIdsRef.current.map((id) => String(id).trim()).filter(Boolean),
					);
					prevSentPandaSubsRef.current = new Set();
				...
				ws.onmessage = (event) => {
					...
					try {
						...
						if (
							...
							venueStatusRef.current.set(mid, message.venues as VenueStatusInfo[]);
	...
	useEffect(() => {
		...
		const want = new Set(activePandaMatchIds.map((id) => String(id).trim()).filter(Boolean));
