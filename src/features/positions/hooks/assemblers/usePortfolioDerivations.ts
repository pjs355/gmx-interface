...
function computeOpenPositionsValue(umbrellaPositions: UmbrellaPositions[]): number {
...
function computeUnclaimedWinningsPayoutTotal(
	resolvedUmbrellaPositions: UmbrellaPositions[],
): number {
...
export function usePortfolioDerivations({
	...
	const openPositionsValue = useMemo(
		() => computeOpenPositionsValue(umbrellaPositions),
		[umbrellaPositions],
	);

	const unclaimedWinningsPayoutTotal = useMemo(
		() => computeUnclaimedWinningsPayoutTotal(resolvedUmbrellaPositions),
		[resolvedUmbrellaPositions],
	);
	...
	const combinedOrders = useMemo(() => {
		...
		for (const up of umbrellaPositions) {
			for (const mp of up.markets) {
				const qid =
					mp.market._id || mp.market.questionId || (mp.market as { marketId?: string }).marketId;
