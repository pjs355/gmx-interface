	...
	return useMemo(() => {
		...
		const levelUpUmbrellas: UmbrellaPositions[] = umbrellas
			.map((umbrella) => {
				...
				const { yes: listingYes, no: listingNo } = getListingYesNoPricesForUmbrella(
					umbrella,
					oddsMarkets,
				);
				const hasListingPrice = typeof listingYes === "number" || typeof listingNo === "number";
				const processedMarkets: MarketPosition[] = markets
					.map((market) => {
						...
						const yesPrice = typeof listingYes === "number" ? listingYes : null;
						const noPrice = typeof listingNo === "number" ? listingNo : null;
						const yesValue =
							hasListingPrice && typeof listingYes === "number" ? yesBalance * listingYes : 0;
						const noValue =
							hasListingPrice && typeof listingNo === "number" ? noBalance * listingNo : 0;
				...
				const matchVenuePositions = (
					...
					for (const pv of venuePositions) {
						...
						const predictTitleHint =
							venue === "predictfun"
								? (predictDetail?.question ?? predictDetail?.title ?? "").trim() || undefined
								: undefined;
						...
						if (matched && matched._id === umbrella._id) {
							...
							const monitorForUmbrella = oddsMarkets.find(
								(mm) => String(mm.umbrellaId ?? "").trim() === String(umbrella._id).trim(),
							);
							...
							if (venue === "predictfun") {
								...
								const yP = isYesForPredict ? (liveYesPrice ?? pv.currentPrice) : null;
								const nP = isYesForPredict ? null : (liveNoPrice ?? pv.currentPrice);
								const yV = yP !== null ? pv.shares * yP : isYesForPredict ? pv.currentValue : 0;
								const nV = nP !== null ? pv.shares * nP : isYesForPredict ? 0 : pv.currentValue;
								overrides = {
									yesPrice: yP,
									noPrice: nP,
									yesValue: yV,
							...
							const displayOverride = matched.displayName?.trim() || undefined;
							matches.push(
