...
import { type VenueId, type VenuePosition } from "@/types/trading/venuePosition";
import {
	type MarketPosition,
...
export function useResolvedUmbrellaPositions({
	...
	const enrichWithHistoryUmbrella = (raw: VenuePosition): VenuePosition => {
		...
		return {
			...raw,
			levelUpUmbrellaId: enriched.levelUpUmbrellaId?.trim() || raw.levelUpUmbrellaId,
			levelUpUmbrellaDisplayName:
				enriched.levelUpUmbrellaDisplayName?.trim() || raw.levelUpUmbrellaDisplayName,
			marketTitle: enriched.marketTitle?.trim() || raw.marketTitle,
		};
	};
	/**
	...
	const predictLookupForWinnings = useMemo(
		() =>
			predictUmbrellaLookup ?? buildPredictUmbrellaLookup(oddsMonitorMarkets ?? null, umbrellas),
	...
	return useMemo(() => {
		...
		Object.entries(resolvedMarketsByUmbrella).forEach(([umbrellaId, resolvedMarkets]) => {
			...
			if (!umbrella) {
				...
				umbrella = {
					_id: umbrellaId,
					displayName:
						(firstMarket as PredictionMarket & { umbrellaName?: string })?.umbrellaName ||
			...
			const res = resolvedMarkets
				...
				.filter((mp) => {
					...
					if (isMarketClaimAcked(mp.market as PredictionMarket, claimedMarkets)) return false;
					const outcome = String(
						(mp.market as { resolvedOutcome?: string }).resolvedOutcome || "",
					).toLowerCase();
					if ((outcome === "yes" && mp.yesBalance > 0) || (outcome === "no" && mp.noBalance > 0)) {
						return true;
					}
					...
					if (userDataLoading && balanceId && !tokenBalances.has(String(balanceId))) {
						return true;
					}
					return false;
				})
		...
		const appendVenueWinnings = (
			...
			for (const [, positions] of byGroup) {
				...
				const predictFallbackLabel =
					venue === "predictfun"
						? shortPredictFunMarketTitleForPortfolio(firstWinHint || first.marketTitle) ||
							first.marketTitle
						: stripUmbrellaDisplayPrefix(first.marketTitle).trim() || first.marketTitle;
				...
				const marketsPre: MarketPosition[] = positions.map((pv) => {
					...
					const polyWinRow =
						venue === "polymarket"
							? findMatchedMarketByPolyConditionId(oddsMarkets, pv.conditionId)
							: null;
					const polyWinLabels =
						venue === "polymarket"
					...
					const polyWinInf =
						venue === "polymarket" && polyWinRow && polyWinLabels
							...
							: null;
					const lxNegParent = (
						venue === "limitless" ? String(pv.negRiskParentConditionId ?? "").trim() : ""
					).trim();
					const lxIsNegRisk =
						venue === "limitless" && (pv.isNegRisk === true || Boolean(lxNegParent));
					const isYes =
						venue === "limitless"
					...
					const labelOutcomeName =
						venue === "predictfun" ? (inferredW?.teamName ?? pv.outcome) : pv.outcome;
					const rowLabel = getPredictPositionRowLabel(
						titleForLabel,
						labelOutcomeName,
						side,
						venue === "dflow" && pv.dflowYesSubTitle?.trim() && pv.dflowNoSubTitle?.trim()
							? {
					...
					const dflowRedeemShares = venue === "dflow" ? pv.shares : undefined;
					/**
					 ...
					 */
					const polyAssetTokenId = venue === "polymarket" ? pv.tokenId : undefined;
					const polyIsNegRisk = venue === "polymarket" ? pv.isNegRisk === true : undefined;
					/**
					...
					return {
						market: {
							...
							...(dflowRedeemShares != null ? { _dflowRedeemShares: dflowRedeemShares } : {}),
							...(polyAssetTokenId != null ? { _polyAssetTokenId: polyAssetTokenId } : {}),
							...
							...(venue === "limitless"
								? {
										...
										...(typeof pv.marketStatus === "string" && pv.marketStatus.trim()
											? { _limitlessMarketStatusApi: pv.marketStatus.trim() }
											: {}),
										...(typeof pv.eventSlug === "string" && pv.eventSlug.trim()
											? { _limitlessMarketSlug: pv.eventSlug.trim() }
											: {}),
										...(pv.limitlessVenueExchange
											? {
													_limitlessVenueExchange: pv.limitlessVenueExchange,
												}
											: {}),
										...(pv.limitlessVenueAdapter
											? {
													_limitlessVenueAdapter: pv.limitlessVenueAdapter,
												}
											: {}),
										...(pv.limitlessCollateralAddress
											? {
													_limitlessCollateralAddress: pv.limitlessCollateralAddress,
												}
											: {}),
									}
								: {}),
						...
						if (venue === "limitless" && String(m._id).startsWith(`${idPrefix}-`)) {
							...
							if (pvRow) {
								rawVenueRow = {
									...
									limitlessPartnerRedeemableSignal: pvRow.limitlessPartnerRedeemableSignal,
				...
				const markets: MarketPosition[] = marketsPre.filter(
					(mp) => !isMarketClaimAcked(mp.market as PredictionMarket, claimedMarkets),
				);
				if (markets.length > 0) {
		 ...
		 */
		const venueUmbrellaIdForGroup = (raw: VenuePosition, venue: VenueId): string | null => {
			const p = enrichWithHistoryUmbrella(raw);
			const hintDetail =
				venue === "predictfun" && p.numericMarketId != null
					? predictMarketDetails.get(p.numericMarketId)
					: undefined;
			const hint = (hintDetail?.question ?? hintDetail?.title ?? "").trim() || null;
