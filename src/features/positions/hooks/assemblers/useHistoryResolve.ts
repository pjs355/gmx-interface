...
import { patchDflowVenuePositionOutcomes } from "@/features/trading/venues/dflow/portfolio/dflowPositionsApi";
import {
...
} from "@/features/trading/umbrellaVenueResolveKey";
 ...
 */
export function useHistoryResolve(args: UseHistoryResolveArgs): UseHistoryResolveResult {
	const {
	...
	const venueHistoryResolveQueries = useMemo(() => {
		...
		for (const item of venueHistoryRawItems) {
			...
			const linkedDocPresent = linkedId ? activeUmbrellaIds.has(linkedId) : false;
	...
	const historyVenueUmbrellaResolveQuery = useQuery({
		queryKey: ["umbrella-resolve-venue-history", "payloads", venueHistoryResolveQueriesKeyStable],
		queryFn: async () =>
			postUmbrellaResolveExchangeKeysChunked(
				(body) =>
		...
		enabled: Boolean(authenticated && effectiveAccount && venueHistoryResolveQueries.length > 0),
	...
	useEffect(() => {
		...
		if (historyVenueUmbrellaResolveQuery.isError) {
			console.warn("[FULL_HISTORY_RESOLVE] POST /api/umbrellas/resolve-venue-history failed", {
		...
		if (d?.success && venueHistoryResolveQueries.length > 0) {
			...
			console.info("[FULL_HISTORY_RESOLVE]", {
				...
				sampleQueries: venueHistoryResolveQueries.slice(0, 8).map((q) => ({
					venue: q.venue,
					clientKey: q.clientKey.length > 96 ? `${q.clientKey.slice(0, 96)}…` : q.clientKey,
			...
			if (hits.length === 0) {
				console.warn("[FULL_HISTORY_RESOLVE] batch OK but zero umbrella hits", {
					keyCount: venueHistoryResolveQueries.length,
					sampleKeys: venueHistoryResolveQueries.map((x) => x.clientKey).slice(0, 12),
				});
			}
		} else if (d?.success) {
			const predictRowsMissingResolveClientKey = venueHistoryRawItems.filter(
	...
	const venueHistory = useMemo(() => {
		...
		if (batch && Object.keys(batch).length > 0) {
			rows = venueHistoryRawItems.map((item) => {
				...
				return {
					...
					levelUpUmbrellaDisplayName: item.levelUpUmbrellaDisplayName?.trim() || hit.displayName,
		...
		if (polyWireToUmbrellaIdFromBatch.size > 0) {
			rows = rows.map((item) => {
				...
				const dn = doc?.displayName ? stripUmbrellaDisplayPrefix(doc.displayName).trim() : "";
				return {
					...item,
		...
		const mappedRows = rows.map((item) => {
			...
			if (item.venue === "dflow") {
				...
				const apiTitle = stripUmbrellaDisplayPrefix(item.levelUpUmbrellaDisplayName ?? "").trim();
			...
			const histDetail =
				item.numericMarketId != null ? predictMarketDetails.get(item.numericMarketId) : undefined;
			const histHint = (histDetail?.question ?? histDetail?.title ?? "").trim() || undefined;
		...
		const catalogColumnMap = buildDflowPortfolioColumnMapFromCatalog(historyCatalogUmbrellas);
		const outcomeMintToUmbrella = buildUmbrellaLookupByDflowOutcomeMint(historyCatalogUmbrellas);
		const eventTickerLookup = buildUmbrellaLookupByDflowEventTicker(historyCatalogUmbrellas);
	...
	const historyUmbrellaResolveSettled =
		...
		!(authenticated && effectiveAccount) ||
		historyVenueUmbrellaResolveQuery.isError ||
