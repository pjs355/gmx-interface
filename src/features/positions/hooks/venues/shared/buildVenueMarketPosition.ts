...
import { inferPolymarketYesNoFromToken } from "@/features/trading/venues/polymarket/trade/polyPositionSide";
...
export function buildVenueMarketPosition(
	...
	const predictInferred =
		venue === "predictfun"
			? inferPredictSideFromMarketDetail(predictMarketDetail ?? undefined, pv.tokenId)
			: null;
	...
	const limitlessCatalogSide =
		venue === "limitless" && limitlessCatalogWire
			? inferLimitlessCatalogYesColumn(pv.tokenId, pv.eventSlug, limitlessCatalogWire)
			: null;
			...
			: pv.shares > 0 && (pv.avgPrice || pv.cost)
				? [
						buildSyntheticOrder(
							...
							pv.cost,
							pv.historyTradeAt,
							venue === "dflow" ? (pv as VenuePosition).dflowTradeSideLabel : undefined,
							String(pv.tokenId),
						),
					]
				: [];
	...
	const noPrice =
		overrides?.noPrice !== undefined
			...
			: isYes
				...
				: pv.currentPrice;
	const yesValue =
		overrides?.yesValue !== undefined ? overrides.yesValue : isYes ? pv.currentValue : 0;
	const noValue =
		overrides?.noValue !== undefined ? overrides.noValue : isYes ? 0 : pv.currentValue;
	...
	return {
		market: {
			...
			conditionId: typeof pv.conditionId === "string" ? pv.conditionId : undefined,
			/** Dedup Positions strip vs History (same encoding as winnings rows). */
			_venueHeldTokenId: String(pv.tokenId),
			...(venue === "polymarket" ? { _polyAssetTokenId: pv.tokenId } : {}),
		...
		predictOutcomeLabelYes:
			venue === "predictfun" && isYes ? (predictInferred?.teamName ?? pv.outcome) : undefined,
