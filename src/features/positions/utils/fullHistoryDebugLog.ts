...
export type FullHistoryUnifiedBlock = {
	...
	luMarkets?: Array<{
		market: { _id?: string; questionId?: string; marketId?: string; displayName?: string };
		yes: string;
		no: string;
	}>;
};
...
export function serializeVenuePositionVerbose(pos: VenuePosition): Record<string, unknown> {
	...
	return {
		...
		historyFills: fills && fills.length > 0 ? fills.map(serializeFill) : undefined,
	};
}
	...
	return Object.entries(resolved).map(([umbrellaId, markets]) => {
		...
		for (const m of markets ?? []) {
			...
			if (orderQids.has(marketId)) withOrder++;
				...
				missing.push({
					marketId,
					displayName: String(m?.displayName ?? m?.question ?? m?.umbrellaName ?? "").trim(),
				});
			}
		}
		return {
...
function snapshot(
	...
	const trades = venueHistory.map((pos) => {
		...
		return {
			...
			pandascoreMatchId:
				(umb as { pandascore_matchId?: string } | undefined)?.pandascore_matchId ?? null,
...
export function logFullHistoryDebug(params: LogFullHistoryDebugParams): void {
	...
	const ordersLu = orders.filter((o) => !o.venue || String(o.venue).toLowerCase() === "levelup");
	const ordersOtherVenue = orders.filter(
		(o) => o.venue && String(o.venue).toLowerCase() !== "levelup",
	);

	const ordersSerialized = orders.slice(0, MAX_LEVEL_UP_ORDERS_LOG).map(serializeLevelUpOrder);
	...
	const payload = {
		...
		umbrellaCatalogSummary: params.umbrellas.map((u) => ({
			...
			originalChildCount: Array.isArray(u.originalChildren) ? u.originalChildren.length : 0,
		})),
		resolvedMarketsCoverage: buildResolvedMarketsCoverage(params.resolvedMarketsByUmbrella, orders),
