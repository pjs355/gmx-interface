...
export function shortTeamDisplayName(raw: string): string {
	...
	if (segments.length > 1) {
		...
		if (
			tail.length >= 8 ||
			/\b(bo\d|playoff|playoffs|group stage|season|dracula|masters|championship)\b/i.test(tail)
		) {
			s = head;
		}
	}
...
export function pickResolvedWinnerFromMarkets(
	...
	if (matchWinnerM) {
		const w = tryResolvedMarketWinnerLabel(matchWinnerM, umbrellaDisplayName);
		if (w) return w;
	}
...
export function resolveCanonicalMatchWinner(args: {
	...
	if (sample) {
		...
		const pair =
			parseVsTeamsFromTitle(title) || (umbrellaTitle ? parseVsTeamsFromTitle(umbrellaTitle) : null);
