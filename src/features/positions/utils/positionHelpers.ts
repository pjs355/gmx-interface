...
import type { VenueHistoryFill, VenueId, VenuePosition } from "@/types/trading/venuePosition";
import { venueDisplayLabel } from "@/types/trading/venuePosition";
import { inferVenueHistoryYesNoSide } from "@/features/positions/utils/historyOutcomeWinner";
...
export function buildSyntheticOrder(
	...
	return {
		...
		tokenValue: shares,
		venue,
		...(positionDisplayLabel?.trim() ? { positionDisplayLabel: positionDisplayLabel.trim() } : {}),
	};
}
 ...
 */
function shareWeightedMarkPrice(markets: MarketPosition[], leg: "yes" | "no"): number | null {
	let sumPxSh = 0;
	let sumSh = 0;
...
export function mergeMarketPositions(markets: MarketPosition[]): MarketPosition[] {
	...
	const impliedYesFromValue = totalYesShares > 0 && yesValue > 0 ? yesValue / totalYesShares : null;
	const impliedNoFromValue = totalNoShares > 0 && noValue > 0 ? noValue / totalNoShares : null;
	...
	const effShares =
		pos.shares > 0
			...
			: pos.avgPrice != null &&
				  ...
				  pos.cost != null &&
				  pos.cost > 0
				? pos.cost / (pos.avgPrice > 1 && pos.avgPrice <= 100 ? pos.avgPrice / 100 : pos.avgPrice)
				: 0;
 ...
 */
export function venueHistoryPositionToSyntheticOrders(pos: VenuePosition): ProcessedOrder[] {
 ...
 */
export function venueHistorySyntheticUmbrellaId(title: string, positions: VenuePosition[]): string {
