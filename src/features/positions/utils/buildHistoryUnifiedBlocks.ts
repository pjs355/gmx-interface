...
function collectLuMarketsForUmbrella(
	...
	const pushIfHasOrders = (market: any) => {
		...
		const dedupeKey = market?._id ? String(market._id).trim().toLowerCase() : candidates[0];
		...
		const rawQ =
			...
			hitKey;
		const fa = getFinalAmount(orders as ProcessedOrder[], String(rawQ).trim());
...
function blockLuMarketsCoverQuestionNorm(block: HistoryUnifiedBlock, qidNorm: string): boolean {
...
function mergeOrphanLevelUpFillsIntoBlocks(
	...
	const findMarketForQuestion = (qidNorm: string): { umbrellaId: string; market: any } | null => {
		...
		if (!block) {
			const umb =
				...
				({
					_id: umbrellaKey,
					displayName:
						(catalogMarket as { umbrellaName?: string } | undefined)?.umbrellaName ||
						`Umbrella ${umbrellaKey.slice(0, 8)}…`,
					children: [],
					originalChildren: [],
		...
		if (catalogMarket != null) {
			...
			block.luMarkets.push({
				market: {
					...
					...(header ? { displayName: header, question: header } : {}),
				},
				yes: fa.yesShares.toString(),
				no: fa.noShares.toString(),
			});
...
export function buildHistoryUnifiedBlocks({
	...
	for (const umbrellaId of umbrellaIds) {
		...
		if (!umb) {
			...
			umb = {
				_id: umbrellaId,
				displayName: resolvedList?.[0]?.umbrellaName || `Umbrella ${umbrellaId.slice(0, 8)}…`,
				children: [],
				originalChildren: [],
				createdAt: new Date().toISOString(),
				updatedAt: new Date().toISOString(),
	...
	for (const pos of venueHistory) {
		...
		const dn =
			stripUmbrellaDisplayPrefix(pos.levelUpUmbrellaDisplayName ?? pos.marketTitle).trim() ||
			pos.marketTitle;
