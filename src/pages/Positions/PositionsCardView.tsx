        ...
        <div key={umbrella._id} className="umbrella-card">
          {markets.map(({ market, yes, no }: any) => {
            ...
            return rows.map(({ side, amount }) => {
              ...
              const payoutIfCorrect = isNaN(sharesNum) ? null : sharesNum;
              ...
              const effectiveAvgPrice = (sideAgg && sideAgg.avgPrice !== null) ? sideAgg.avgPrice : null;
                ...
                >
                  ...
                  >
                    ...
                    <div style={{ flex: 1, textAlign: "right" }}>
                      ...
                      <div style={{ color: "#fff", fontSize: 18, fontWeight: 700 }}>
                        <span className={softLoading ? "soft-blur" : undefined}>
                          {marketValue === null || marketValue === undefined || isNaN(marketValue) ? "—" : formatCurrency(marketValue)}
                        </span>
                      </div>
                    </div>
                    <button
                    ...
                    >
                      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                        ...
                        <div style={{ display: "flex", justifyContent: "space-between" }}>
                          ...
                          <span style={{ color: "#fff", fontSize: 13, fontWeight: 600 }}>
                            <span className={softLoading ? "soft-blur" : undefined}>
                              {effectiveAvgPrice === null ? "—" : toCentsString(effectiveAvgPrice)}
                            </span>
                          </span>
                        </div>
                        <div style={{ display: "flex", justifyContent: "space-between" }}>
                          ...
                          <span style={{ color: "#fff", fontSize: 13, fontWeight: 600 }}>
                            <span className={softLoading ? "soft-blur" : undefined}>
                              {effectiveCost === null || effectiveCost === undefined ? "—" : formatCurrency(effectiveCost)}
                            </span>
                          </span>
                        </div>
                        <div style={{ display: "flex", justifyContent: "space-between" }}>
                          ...
                          <span style={{ color: "#fff", fontSize: 13, fontWeight: 600 }}>
                            <span className={softLoading ? "soft-blur" : undefined}>
                              {payoutIfCorrect === null || payoutIfCorrect === undefined ? "—" : formatCurrency(payoutIfCorrect)}
                            </span>
                          </span>
                        </div>
                        <div style={{ display: "flex", justifyContent: "space-between" }}>
                          <span style={{ color: "#888", fontSize: 13 }}>Total Return</span>
                          <span style={{ color: totalReturnColor, fontSize: 13, fontWeight: 600, whiteSpace: "nowrap" }}>
