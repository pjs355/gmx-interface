...
import type { UmbrellaPositions, MarketPosition } from "@/pages/Positions/utils/positionHelpers";
 ...
 */
function accumulateFromResolvedHoldings(resolved: UmbrellaPositions[], qb: Set<string>, tok: Set<string>) {
...
/** @deprecated Prefer {@link buildHistoryHoldingsBlockedKeys} */
export function buildHistoryExclusionFromResolvedPositions(
	resolved: UmbrellaPositions[],
): HistoryHoldingsBlockedKeys {
	const questionKeys = new Set<string>();
	const tokenIds = new Set<string>();
	accumulateFromResolvedHoldings(resolved, questionKeys, tokenIds);
	return { questionKeys, tokenIds };
}

export type UnifiedHistoryBlockForFilter = {
	id: string;
	luMarkets: Array<{ market: { _id?: string; questionId?: string; marketId?: string; conditionId?: string } }>;
	venuePositions: VenuePosition[];
};

function luMarketTouchesBlocked(market: UnifiedHistoryBlockForFilter["luMarkets"][number]["market"], qb: Set<string>): boolean {
...
export function filterUnifiedHistoryBlocksByOpenPositions<
	T extends UnifiedHistoryBlockForFilter,
>(blocks: T[], blocked: HistoryHoldingsBlockedKeys): T[] {
	const out: T[] = [];
	for (const block of blocks) {
		const luMarkets = block.luMarkets.filter((row) => !luMarketTouchesBlocked(row.market, blocked.questionKeys));
