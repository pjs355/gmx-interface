...
export function usePredictBundle({
	...
	const redeemEventsByToken = useMemo(
		() => predictRedeemEventsByToken(activity),
		[activity],
	);
	...
	const { active, winnings, history } = useMemo(() => {
		...
		for (const [tokenId, redeems] of redeemEventsByToken) {
			...
			const outcomeName =
				last.outcome?.name ?? outcomeNameFromDetail ?? "Yes";
			...
			const pnl =
				costOut != null && Number.isFinite(costOut)
					? payout - costOut
					: null;
			const pnlPercent =
				costOut != null && costOut > 0 && pnl != null
					? (pnl / costOut) * 100
					: null;
