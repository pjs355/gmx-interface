    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        console.log("Steam ID check:", {
          ...
          hasSteam: !!(result.data.steamId || result.data.cs?.steamId64),
        });
      }
    } catch (error) {
  ...
  return (
    <div style={{ padding: 24, color: "white" }}>
      ...
      <div style={{ marginBottom: 24, opacity: 0.9, lineHeight: 1.5 }}>
        Link your gaming accounts to verify matches and participate in gaming predictions. Your accounts are used solely
        for match verification and prediction participation.
      </div>
      ...
      <div style={{ marginBottom: 32 }}>
        ...
        >
          ...
          <div style={{ marginBottom: 16, opacity: 0.8, fontSize: 14 }}>{getPlatformDescription("steam")}</div>
          ...
          {steamLinkStatus === "success" && (
            ...
            >
              ...
              <div style={{ fontSize: 14, opacity: 0.8 }}>
                Your Steam account has been connected. You can now participate in CS:GO/CS2 predictions.
              </div>
            </div>
          )}
              ...
              <div>
                ...
                <div style={{ fontSize: 12, opacity: 0.6 }}>
                  {linkedAccounts.find((acc) => acc.platform === "steam")?.linkedAt
                    ? `Linked on ${new Date(linkedAccounts.find((acc) => acc.platform === "steam")?.linkedAt || "").toLocaleDateString()}`
                    : "Connected via Steam"}
                </div>
              </div>
              <button
          ...
          {linkedAccounts.find((acc) => acc.platform === "lol") ? (
            ...
            >
              <div>
                ...
                <div style={{ fontSize: 12, opacity: 0.6 }}>
                  Linked on{" "}
                  {new Date(linkedAccounts.find((acc) => acc.platform === "lol")?.linkedAt || "").toLocaleDateString()}
                </div>
              </div>
              <button
      ...
      {linkedAccounts.length > 0 && (
        <div style={{ marginTop: 32 }}>
          <h3 style={{ marginBottom: 16, fontSize: 18, fontWeight: 600 }}>Linked Accounts Summary</h3>
            ...
            {linkedAccounts.map((account, index) => (
              ...
              >
                <div style={{ display: "flex", alignItems: "center" }}>
                  <span style={{ fontSize: 20, marginRight: 12 }}>{getPlatformIcon(account.platform)}</span>
                  <div>
                    <div style={{ fontWeight: 600 }}>{getPlatformName(account.platform)}</div>
                    <div style={{ fontSize: 14, opacity: 0.8 }}>{account.username}</div>
                  </div>
                </div>
