...
import PredictionMarketTradeBox, { type PredictionMarketTradeBoxHandle } from "components/PredictionMarketTradeBox/PredictionMarketTradeBox";
import { TradeBoxTestRunner, type TestResult, type TestScenario } from "utils/TradeBoxTestRunner";
...
export default function TradeBoxTest() {
  ...
  const [testRunner] = useState(() => new TradeBoxTestRunner({
    ...
    onTestComplete: (result) => {
      ...
      setTestLogs(prev => [...prev, {
        ...
        message: `Completed: ${result.scenarioName} - ${result.success ? 'PASSED ✅' : 'FAILED ❌'}`,
      }]);
    },
  ...
  const [selectedScenarioType, setSelectedScenarioType] = useState<'essential' | 'all' | 'market-only'>('essential');
  ...
  const getCurrentBalances = async () => {
    ...
    console.log('💰 LIVE balances from ref:', { usdc, yes, no, timestamp: Date.now() });
  ...
  const executeTradeScenario = async (scenario: TestScenario) => {
    ...
    try {
      ...
      if (!stateCorrect) {
        ...
        throw new Error(`Component state did not match after 5 seconds. Expected position=${scenario.position} but got ${finalState.selectedPosition}`);
      }
  ...
  }, [authenticated, ready, signer, signerAddress, market, orderbook, scenarios, account, testRunner, executeTradeScenario, getCurrentBalances]);
    ...
    return (
      <div className="trade-box-test">
        <div className="test-header">
          ...
          <button onClick={() => navigate('/predictions')}>Go to Predictions</button>
        </div>
      </div>
    );
  }
      ...
      <div className="test-content">
        ...
        <div className="test-panel trade-box-panel">
          ...
          <div className="current-balances">
            ...
            <div className="balance-row">
              <span>USDC:</span>
              <span className="balance-value">${parseFloat(usdcBalance || '0').toFixed(2)}</span>
            </div>
            {market && getTokenBalance && (() => {
        ...
        <div className="test-panel results-panel">
          ...
          <div className="test-controls">
            ...
            {!authenticated && (
              <p className="warning">Please log in with Privy to run tests</p>
            )}
