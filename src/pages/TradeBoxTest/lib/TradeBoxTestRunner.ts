...
export class TradeBoxTestRunner {
	...
	async runAllTests(
		...
		_delayBetweenTests: number = 45000, // Increased to 45 seconds for balance updates
	) {
		if (this.isRunning) {
			...
			try {
				...
				if (scenario.side === "sell") {
					...
					const availableShares =
						scenario.position === "yes" ? balanceBefore.yes : balanceBefore.no;
	...
	private compareResults(
		...
		if (expected.usdSpent !== undefined && actual.usdSpent !== undefined) {
			...
			details.push(
				`USD Spent: Expected ${expected.usdSpent.toFixed(2)}, Got ${actual.usdSpent.toFixed(2)} (${
					usdMatch ? "✅" : "❌"
				})`,
			);
		}
		...
		if (expected.usdReceived !== undefined && actual.usdReceived !== undefined) {
			...
			details.push(
				`USD Received: Expected ${expected.usdReceived.toFixed(
					2,
				)}, Got ${actual.usdReceived.toFixed(2)} (${usdMatch ? "✅" : "❌"})`,
			);
		}
		...
		if (expected.avgPrice !== undefined && actual.avgPrice !== undefined) {
			...
			details.push(
				`Avg Price: Expected ${expected.avgPrice.toFixed(4)}, Got ${actual.avgPrice.toFixed(4)} (${
					priceMatch ? "✅" : "❌"
				})`,
			);
		}
