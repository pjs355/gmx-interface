...
export function useTradeBoxSorExecuteActions(
	...
	const dflowFirstMintRefreshTimersRef = useRef<
	...
	const handleSorExecute = useCallback(() => {
		...
		if (executableRoute && !sorRouteExpired) {
			...
			if (!legsMatchVenueTab) {
				console.error(
					"[SOR] execute blocked: route legs do not match selected venue tab",
					{
						tradingVenue: tv,
			...
			void sorExecution
				...
				.then((res) => {
					if (res == null) {
						console.error(
							"[SOR] execute settled: null result (execute ignored or bug)",
						);
						return;
					}
					const summary = {
	...
	useEffect(() => {
		...
		if (status === "complete" && everyLegFilled) {
			...
			const postTradeCommon = {
				...
				refreshLevelUpOrders: () => refreshOrders({ force: true }),
