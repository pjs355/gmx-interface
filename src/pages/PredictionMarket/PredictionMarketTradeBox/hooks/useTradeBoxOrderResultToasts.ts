...
/** Toast + hard-dismiss for `state.orderResult` (umbrella page + home inline dock). */
export function useTradeBoxOrderResultToasts(
	orderResult: TradeBoxCoreState["orderResult"],
): void {
	const orderResultToastSigRef = useRef<string | null>(null);
	const orderResultToastDismissTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
		null,
	);
	useEffect(() => {
		if (!orderResult) {
