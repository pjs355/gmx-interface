...
function PredictionMarketContent() {
  ...
  const [questionOrderbooks, setQuestionOrderbooks] = useState<{[questionId: string]: any}>({});
  ...
  useEffect(() => {
    ...
    for (const q of (qs as any[]).filter(Boolean)) {
      ...
      if (qid) seeded[qid] = getOrderbookForQuestion(umbrellaFromContext._id, qid);
    }
    setQuestionOrderbooks(seeded);
    setLoading(false);
  }, [umbrellaId, umbrellas, getUmbrellaById, getQuestionsForUmbrella, getOrderbookForQuestion, navigate]);
  ...
  const fetchAllOrderbooks = useCallback(async (qs: PredictionMarket[]) => {
    ...
    for (const q of qs as any[]) {
      const qid = (q as any)._id || (q as any).questionId || (q as any).marketId;
      if (qid) updated[qid] = getOrderbookForQuestion(umbrella._id, qid);
    }
  ...
  useEffect(() => {
    if (!hasUserSelectedMarket && !hasProcessedStoredSelection && sortedQuestions.length > 0) {
      ...
      if (storedMarketId) {
        ...
        targetMarket = sortedQuestions.find(question => {
      ...
      if (targetMarket && (!activeMarket || getMarketId(activeMarket) !== getMarketId(targetMarket))) {
        setActiveMarket(targetMarket);
      }
    }
  }, [hasUserSelectedMarket, hasProcessedStoredSelection, sortedQuestions, activeMarket, getMarketId]);
  ...
  useEffect(() => {
    ...
    const refreshActive = async () => {
      ...
      setQuestionOrderbooks(prev => ({ ...prev, [activeMarketId]: ob }));
    };
  ...
  }, [activeMarket, umbrella, getMarketId, refreshOrderbook, getOrderbookForQuestion]);
