...
export default function ListMarket({ onEdit }: { onEdit: (umbrella: Umbrella) => void }) {
  ...
  return (
    <div style={{ padding: 24, color: "white" }}>
      ...
      <div style={{ display: "grid", gap: 12 }}>
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
              <div>
                ...
                <div style={{ fontSize: 12, opacity: 0.8 }}>ID: {u._id}</div>
                {u.children?.length ? (
                  <div style={{ fontSize: 12, opacity: 0.8 }}>Questions: {u.children.length}</div>
                ) : (
        ...
        {!loading && filtered.length === 0 && <div style={{ opacity: 0.8 }}>No umbrellas found.</div>}
      </div>
    </div>
  );
}
