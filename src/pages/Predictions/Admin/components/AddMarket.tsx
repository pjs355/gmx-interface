...
export default function AddMarket() {
  ...
  const [form, setForm] = useState<AddMarketForm>({
    ...
    umbrellaDisplayName: "",
    umbrellaRule: "",
    yesColor: "#22c55e", // emerald-500
    noColor: "#ef4444", // red-500
    isEvent: false,
    eventDate: "",
    tags: ["POKEMON"],
  });
  ...
  const uploadImageToFirebase = async (file: File, imageType: "image1" | "image2"): Promise<string> => {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const payload: any = {
        ...
        tags: form.tags,
        yesColor: form.yesColor,
        noColor: form.noColor,
        umbrellaId: form.selectedUmbrellaId || undefined,
        umbrellaDisplayName: !form.selectedUmbrellaId ? form.umbrellaDisplayName || undefined : undefined,
        umbrellaRule: !form.selectedUmbrellaId ? form.umbrellaRule || undefined : undefined,
        rule: !form.selectedUmbrellaId ? form.umbrellaRule || undefined : undefined,
      ...
      if (image1) {
        payload.image1Url = await uploadImageToFirebase(image1, "image1");
      }

      if (image2) {
        payload.image2Url = await uploadImageToFirebase(image2, "image2");
      }

      const accessToken = typeof getAccessToken === "function" ? await getAccessToken() : undefined;
      const resp = await fetch("https:prediction-api-production.up.railway.app/admin/markets", {
        method: "POST",
        headers: {
    ...
    <div style={{ padding: 24, color: "white" }}>
      ...
      <form onSubmit={handleSubmit} style={{ display: "grid", gap: 12, maxWidth: 720 }}>
        <label style={{ display: "grid", gap: 6 }}>
          ...
          {loadingUmbrellas && <span style={{ fontSize: 12, opacity: 0.8 }}>Loading umbrellas...</span>}
        </label>
        ...
        {!form.selectedUmbrellaId && (
          <label style={{ display: "grid", gap: 6 }}>
            ...
            <input
              ...
              onChange={(e) => update("umbrellaDisplayName", e.target.value)}
        ...
        {!form.selectedUmbrellaId && (
          <label style={{ display: "grid", gap: 6 }}>
            ...
            <textarea
              ...
              onChange={(e) => update("umbrellaRule", e.target.value)}
        ...
        {form.selectedUmbrellaId &&
          (() => {
            const selected = umbrellas.find((u) => u._id === form.selectedUmbrellaId);
            ...
            return (
              ...
              >
                <div style={{ marginBottom: 8, fontWeight: 600 }}>
                  Existing questions in "{selected.displayName}" ({children.length})
                </div>
                {children.length === 0 && <div style={{ opacity: 0.8 }}>No questions found under this umbrella.</div>}
                {children.length > 0 && (
                  <ul style={{ listStyle: "disc", paddingLeft: 20, margin: 0, color: "white" }}>
                    {children.map((c) => (
                      <li key={c.questionId} style={{ marginBottom: 6 }}>
                        <span style={{ color: "white" }}>{c.displayName}</span>
                        <span style={{ color: "#9ca3af" }}>{" — "}</span>
                        <span style={{ color: "#9ca3af", fontSize: 12 }}>id: {c.questionId}</span>
                      </li>
                    ))}
                  </ul>
                )}
        ...
        <label style={{ display: "grid", gap: 6 }}>
          <span>Question (Must Be Unique, This is Used For CTF)</span>
          <input
            value={form.question}
            onChange={(e) => update("question", e.target.value)}
            placeholder="e.g. 2025 - Mega Evolution ..."
            required
            style={{
              padding: 8,
              color: "cyan",
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          <label style={{ display: "grid", gap: 6 }}>
            ...
            <input
              type="color"
              value={form.yesColor}
              onChange={(e) => update("yesColor", e.target.value)}
              style={{
                height: 40,
                padding: 0,
                background: "transparent",
          ...
          <label style={{ display: "grid", gap: 6 }}>
            ...
            <input
              type="color"
              value={form.noColor}
              onChange={(e) => update("noColor", e.target.value)}
              style={{
                height: 40,
                padding: 0,
                background: "transparent",
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button
              ...
              style={{
                ...
                border: "1px solid white",
                borderRadius: 6,
                background: form.isEvent ? "transparent" : "rgba(255,255,255,0.2)",
                color: "white",
                cursor: "pointer",
              }}
            >
            ...
            <button
              ...
              style={{
                ...
                border: "1px solid white",
                borderRadius: 6,
                background: form.isEvent ? "rgba(255,255,255,0.2)" : "transparent",
                color: "white",
                cursor: "pointer",
              }}
            >
          ...
          {form.isEvent && (
            <label style={{ display: "grid", gap: 6 }}>
              ...
              <span style={{ fontSize: 12, opacity: 0.8 }}>
                Stored as local time; we can convert to UTC ISO on submit if preferred.
              </span>
            </label>
          )}
        </div>
            ...
            <div style={{ display: "grid", gap: 8 }}>
              ...
              <div style={{ display: "grid", gap: 8 }}>
                ...
                {uploadingImage === "image1" && (
                  <div style={{ fontSize: 12, opacity: 0.8, color: "#8b5cf6" }}>Uploading...</div>
                )}
              </div>
            </div>
