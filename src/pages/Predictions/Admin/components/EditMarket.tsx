...
import type { Umbrella, UmbrellaQuestion } from "../../../../lib/umbrellaDataService";
...
export default function EditMarket({ umbrella, onBack }: { umbrella: Umbrella; onBack: () => void }) {
  const { getAccessToken } = usePrivy();
  const [selectedChild, setSelectedChild] = useState<UmbrellaQuestion | null>(null);
  ...
  async function saveUmbrella() {
    ...
    try {
      ...
      if (umbIsEvent) {
        body.eventDate = umbEventDate ? new Date(umbEventDate).toISOString() : null;
      } else {
        body.eventDate = null;
      }
  ...
  async function settleQuestion() {
    ...
    const questionId = (details?.questionId || selectedChild?.questionId || "").toString();
    if (!questionId) {
  ...
  return (
    <div style={{ padding: 24, color: "white" }}>
      ...
      <label style={{ display: "grid", gap: 6, marginTop: 8, maxWidth: 720 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button
            ...
            style={{
              ...
              border: "1px solid white",
              borderRadius: 6,
              background: umbIsEvent ? "transparent" : "rgba(255,255,255,0.2)",
              color: "white",
              cursor: "pointer",
            }}
          >
          ...
          <button
            ...
            style={{
              ...
              border: "1px solid white",
              borderRadius: 6,
              background: umbIsEvent ? "rgba(255,255,255,0.2)" : "transparent",
              color: "white",
              cursor: "pointer",
            }}
          >
          ...
          <label style={{ display: "grid", gap: 6 }}>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              <input
                ...
                type="datetime-local"
                value={umbEventDate}
                onChange={(e) => setUmbEventDate(e.target.value)}
                style={{
                  padding: 8,
                  color: "cyan",
                  border: "1px solid white",
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
        ...
        {umbSaveMsg && <span style={{ color: "#22c55e" }}>{umbSaveMsg}</span>}
        {umbSaveErr && <span style={{ color: "#ff6b6b" }}>{umbSaveErr}</span>}
      </div>

      <div style={{ marginTop: 16, borderTop: "1px solid rgba(255,255,255,0.2)", paddingTop: 12 }}>
        <div style={{ marginBottom: 8, fontWeight: 600 }}>Questions</div>
        {children.length === 0 && <div style={{ opacity: 0.8 }}>No questions in this umbrella.</div>}
        {children.length > 0 && (
          <div style={{ display: "grid", gap: 8 }}>
            {children.map((c) => (
              <div
              ...
              >
                <div>
                  ...
                  <div style={{ fontSize: 12, opacity: 0.8 }}>id: {c.questionId}</div>
                </div>
                <button
                  type="button"
                  onClick={() => {
      ...
      {selectedChild && (
        <div style={{ marginTop: 16, borderTop: "1px solid rgba(255,255,255,0.2)", paddingTop: 12 }}>
          <div style={{ marginBottom: 8, fontWeight: 600 }}>Editing Question</div>
          ...
          {details && (
            <div style={{ display: "grid", gap: 12, maxWidth: 720 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <label style={{ display: "grid", gap: 6 }}>
                  ...
                  <input
                    ...
                    onChange={(e) => setDetails({ ...details, yesColor: e.target.value })}
                    style={{
                      height: 40,
                      padding: 0,
                      background: "transparent",
                ...
                <label style={{ display: "grid", gap: 6 }}>
                  ...
                  <input
                    ...
                    onChange={(e) => setDetails({ ...details, noColor: e.target.value })}
                    style={{
                      height: 40,
                      padding: 0,
                      background: "transparent",
              ...
              <div style={{ display: "grid", gap: 6 }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                  {AVAILABLE_TAGS.map((tag) => {
                    const selected = Array.isArray(details.tags) && details.tags.includes(tag);
                    return (
                      <button
                        type="button"
                        key={tag}
              ...
              <div style={{ display: "flex", gap: 8, marginTop: 8, order: 1 }}>
                <button
                  type="button"
                ...
                {qSaveMsg && <span style={{ color: "#22c55e" }}>{qSaveMsg}</span>}
                {qSaveErr && <span style={{ color: "#ff6b6b" }}>{qSaveErr}</span>}
              </div>
            </div>
          )}
        </div>
