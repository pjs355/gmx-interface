...
export default function UmbrellaPage() {
	...
	useEffect(() => {
		const loadUmbrellaData = async () => {
			...
			if (storedUmbrella) {
				try {
					const parsedUmbrella = JSON.parse(
						storedUmbrella
					) as Umbrella;
					...
					if (parsedUmbrella._id === umbrellaId) {
						...
						try {
							if (
								parsedUmbrella.children &&
								parsedUmbrella.children.length > 0
							) {
								...
								console.log(
									"⚠️ No children found for umbrella:",
									parsedUmbrella.displayName
								);
								setQuestions([]);
							}
						} catch (error) {
							...
							setError(
								"Failed to load questions for this umbrella"
							);
						}
					} else {
						console.log(
							"❌ Umbrella ID mismatch:",
							parsedUmbrella._id,
							"vs",
							umbrellaId
						);
					...
					console.error(
						"❌ Error parsing stored umbrella data:",
						error
					);
	...
	const navigateToQuestion = (question: PredictionMarket) => {
		if (!umbrella) {
			throw new Error("No umbrella data available for navigation");
		}
