...
function findSmartWalletAddress(linkedAccounts: any[] | undefined): string | undefined {
  return (linkedAccounts || []).find((acct: any) => acct?.type === "smart_wallet")?.address as string | undefined;
}
...
export default function useWallet() {
  ...
  const smartAddress = useMemo(() => findSmartWalletAddress(user?.linkedAccounts), [user?.linkedAccounts]);
  ...
  const embeddedAddress = useMemo(() => {
    const emb = (privyWallets || []).find(
      (w: any) => w?.type === "embedded_wallet" || w?.walletClientType === "privy" || w?.connectorType === "privy"
    );
  ...
  const address = useMemo(() => {
    const resolvedAddress =
      smartAddress ?? embeddedAddress ?? externalAddress ?? anyWalletAddress ?? linkedAccountAddress ?? undefined;
    return resolvedAddress;
  }, [smartAddress, embeddedAddress, externalAddress, anyWalletAddress, linkedAccountAddress]);

  const chainId = base.id;
  const isConnected = Boolean(authenticated && (address || (privyWallets && privyWallets.length > 0)));
  ...
  const dataAddress = useMemo(() => {
    ...
    return smartAddress ?? embeddedAddress ?? externalAddress ?? anyWalletAddress ?? linkedAccountAddress ?? undefined;
  }, [smartAddress, embeddedAddress, externalAddress, anyWalletAddress, linkedAccountAddress]);
  ...
  const getAnySigner = async (): Promise<EthersSigner | undefined> => {
    ...
    const anyWallet = (privyWallets || []).find((w: any) => w?.address && typeof w.getEthereumProvider === "function");
    if (anyWallet) {
