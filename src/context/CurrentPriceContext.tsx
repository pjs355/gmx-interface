...
import { currentPriceService, type MarketPrices } from 'lib/currentPriceService';

interface CurrentPriceContextType {
  getCurrentPrice: (marketId: string, position: 'yes' | 'no') => number | null;
...
export function CurrentPriceProvider({ children }: { children: React.ReactNode }) {
  const [isLoading, setIsLoading] = useState(false);
  const [refreshTrigger, setRefreshTrigger] = useState(0);

  const getCurrentPrice = useCallback((marketId: string, position: 'yes' | 'no'): number | null => {
    ...
    if (!cached) {
      ...
      currentPriceService.getCurrentPrice(marketId, position).catch(console.error);
      return null;
    }
    
    // Check if cache is stale
    if (Date.now() - cached[position].timestamp > 30000) { // 30 seconds
  ...
  const getMarketPrices = useCallback((marketId: string): MarketPrices | null => {
    // Use the service's improved logic directly
    const cached = currentPriceService.getCachedPrices(marketId);
    if (!cached) {
      // Trigger async refresh but return null for now
      currentPriceService.getMarketPrices(marketId).catch(console.error);
      return null;
    }
    
    // Check if cache is stale
    if (Date.now() - cached.yes.timestamp > 30000 || Date.now() - cached.no.timestamp > 30000) {
      // If we have stale data, return it while refreshing in background
  ...
  return (
    <CurrentPriceContext.Provider value={{ 
      getCurrentPrice, 
      getMarketPrices, 
      refreshMarkets, 
      isLoading 
...
export function useCurrentPrices() {
  ...
  if (!context) {
    throw new Error('useCurrentPrices must be used within a CurrentPriceProvider');
  }
  return context;
}
