		...
		try {
			if (!authenticated) {
				...
				if (currentDebugOverride) {
					console.warn(
						"🔧 DEBUG MODE ACTIVE (unauthenticated): Viewing data for",
			...
			const smartLinked: any = ((user as any)?.linkedAccounts || []).find(
			...
			const unifiedAccount =
				smartAddress ||
				embedded?.address ||
				(!hasSmart ? external?.address : undefined) ||
				undefined;
			...
			if (currentDebugOverride) {
				setAccount(currentDebugOverride);
				console.warn(
					"🔧 DEBUG MODE ACTIVE: Spoofing account",
					currentDebugOverride,
					"(real account:",
					unifiedAccount,
					")",
			...
			const nextType: "smart" | "embedded" | "external" | "none" = hasSmart
				? "smart"
				: embedded
					? "embedded"
					: external
...
async function checkTokenBalance(tokenId: string, address?: string): Promise<void> {
	...
	if (!targetAddress) {
		console.error(
			"No address — use checkTokenBalance('tokenId', '0x...') or spoof an account first.",
		);
		return;
	}
