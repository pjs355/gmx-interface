...
export class TradeBoxTestScenarios {
  ...
  private static generateLimitBuyScenarios(
    ...
    const testPrices = [
      ...
      Math.min(99, Math.max(1, Math.floor(bestAsk * 0.90 * 100))), // 10% below (likely to sit in book)
    ];
  ...
  private static generateLimitSellScenarios(
    ...
    const testPrices = [
      ...
      Math.min(99, Math.max(1, Math.floor(bestBid * 1.10 * 100))), // 10% above (likely to sit in book)
    ];
  ...
  static generateEssentialScenarios(orderbook: OrderbookSnapshot): TestScenario[] {
    ...
    if (yesAsks.length > 0) {
      ...
      scenarios.push({
        ...
        amount: 10,
        price: yesLimitPrice,
        expectedOutcome: ExpectedOutcomeCalculator.calculateLimitBuy(10, yesLimitPrice, 'yes', orderbook),
      });
    }
    ...
    if (yesBids.length > 0) {
      ...
      scenarios.push({
        ...
        expectedOutcome: ExpectedOutcomeCalculator.calculateLimitSell(10, yesLimitSellPrice, 'yes', orderbook),
      });
    }
