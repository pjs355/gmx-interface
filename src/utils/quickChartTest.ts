...
(window as any).testCurrentChart = () => {
	...
	const chartElements = document.querySelectorAll(
		'[data-testid="prediction-chart"], .prediction-market-chart'
	);
	...
	cachedMarkets.forEach((market, index) => {
		...
		console.log(
			`Market ${index + 1}: ${market.displayName || market.question}`
		);
...
(window as any).testAllCharts = () => {
	...
	if (markets.length === 0) {
		...
		console.log(
			"💡 This suggests PredictionDataContext hasn't loaded data yet."
		);
		return;
	}
		...
		try {
			...
			if (historicalPrices && historicalPrices.length > 0) {
				...
				const firstTimestamp = Math.min(
					...historicalPrices.map((p) => p.timestamp)
				);
				...
				console.log(
					`   ✅ Historical data: ${historicalPrices.length} points`
				);
				console.log(
					`   📈 Price range: ${minPrice.toFixed(
						3
					)} - ${maxPrice.toFixed(3)}`
				);
				console.log(
				...
				if (minPrice === maxPrice) {
					results.issues.push(
						`${marketName}: Flat price data (${minPrice.toFixed(
							3
						)})`
					);
				}
				if (historicalPrices.length < 5) {
					results.issues.push(
						`${marketName}: Very few data points (${historicalPrices.length})`
					);
				}
			} else {
				results.withoutHistoricalData++;
				console.log(
					`   ❌ No historical data - should show 50% fallback`
				);
				results.issues.push(
					`${marketName}: No historical data available`
				);
			}
		} catch (error) {
	...
	console.log(
		`With historical data: ${results.withHistoricalData} (${Math.round(
			(results.withHistoricalData / results.total) * 100
		)}%)`
	);
	console.log(
		`Without historical data: ${
			...
			(results.withoutHistoricalData / results.total) * 100
		)}%)`
	);
...
(window as any).checkDataFlow = () => {
	...
	if (currentUrl.includes("/predictions/")) {
		...
		const charts = document.querySelectorAll(
			'.prediction-market-chart, [data-testid="prediction-chart"]'
		);
		...
		if (rechartElements.length === 0) {
			console.log(
				"   ⚠️ No Recharts elements found - charts may not be rendering"
			);
		}
	}
...
(window as any).simulateChartProcessing = (questionId: string) => {
	...
	if (!historicalPrices || historicalPrices.length === 0) {
		...
		console.log(
			"💡 Expected behavior: Show 50% line from 1 hour ago to now"
		);
