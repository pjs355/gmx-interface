...
export class TradeBoxTestRunner {
	...
	async runAllTests(
		...
		if (!this.market || !this.orderbook) {
			throw new Error(
				"Market and orderbook must be set before running tests"
			);
		}
		...
		console.log(
			`🚀 Starting test run with ${this.scenarios.length} scenarios`
		);
			...
			try {
				...
				if (scenario.side === "sell") {
					...
					const availableShares =
						scenario.position === "yes"
							? balanceBefore.yes
							: balanceBefore.no;

					if (availableShares < requiredShares) {
						console.warn(
							`⚠️ Insufficient ${scenario.position.toUpperCase()} tokens: need ${requiredShares}, have ${availableShares}`
						);
						throw new Error(
							`Insufficient ${scenario.position.toUpperCase()} tokens: need ${requiredShares}, have ${availableShares}. Skipping test.`
						);
					}
					console.log(
						`✅ Sufficient balance: ${availableShares} ${scenario.position.toUpperCase()} tokens available`
					);
				}
				...
				console.log(
					`⏳ Polling for balance changes (max 60 seconds, will stop early if test passes)...`
				);
				...
				while (attempts < maxAttempts && !testPassed) {
					...
					console.log(
						`🔍 Balance check ${attempts}/${maxAttempts}:`,
						currentBalance
					);
					...
					if (scenario.side === "buy") {
						...
						const usdcSpent =
							balanceBefore.usdc - currentBalance.usdc;
						...
						if (tokensReceived > 0 && usdcSpent > 0) {
							...
							console.log(
								`✅ Test passed! Both balances updated after ${
									attempts * 3
								} seconds!`
							);
							console.log(
								`   Tokens received: ${tokensReceived}, USDC spent: ${usdcSpent}`
							);
						} else if (tokensReceived > 0 || usdcSpent > 0) {
							// Partial update detected, keep polling
							console.log(
								`⏳ Partial update: tokens=${tokensReceived}, usdc spent=${usdcSpent}. Continuing...`
							);
						}
					} else {
						...
						const usdcReceived =
							currentBalance.usdc - balanceBefore.usdc;
						...
						if (tokensSold > 0 && usdcReceived > 0) {
							...
							console.log(
								`   Tokens sold: ${tokensSold}, USDC received: ${usdcReceived}`
							);
						} else if (tokensSold > 0 || usdcReceived > 0) {
							// Partial update detected, keep polling
							console.log(
								`⏳ Partial update: tokens sold=${tokensSold}, usdc received=${usdcReceived}. Continuing...`
							);
						}
					}
				}
				...
				if (!testPassed) {
					console.warn(
						`⚠️ Test did not fully pass after ${
							maxAttempts * 3
						} seconds`
					);
	...
	private calculateActualResults(
		...
		if (scenario.side === "buy") {
			...
			const avgPrice =
				contractsReceived > 0 ? usdSpent / contractsReceived : 0;
			...
			// Calculate average price
			const avgPrice =
				contractsSold > 0 ? usdReceived / contractsSold : 0;
	...
	private compareResults(
		...
		if (
			expected.contractsReceived !== undefined &&
			actual.contractsReceived !== undefined
		) {
			const diff = Math.abs(
				expected.contractsReceived - actual.contractsReceived
			);
			const pctDiff =
				expected.contractsReceived > 0
					? diff / expected.contractsReceived
					: 0;
			contractsMatch = pctDiff <= tolerance;

			details.push(
				`Contracts: Expected ${expected.contractsReceived}, Got ${
					actual.contractsReceived
				} (${contractsMatch ? "✅" : "❌"})`
			);
		}
		...
		if (expected.usdSpent !== undefined && actual.usdSpent !== undefined) {
			...
			const pctDiff =
				expected.usdSpent > 0 ? diff / expected.usdSpent : 0;
			usdMatch = pctDiff <= tolerance;

			details.push(
				`USD Spent: Expected ${expected.usdSpent.toFixed(
					2
				)}, Got ${actual.usdSpent.toFixed(2)} (${
					usdMatch ? "✅" : "❌"
				})`
			);
		}
		...
		if (
			expected.usdReceived !== undefined &&
			actual.usdReceived !== undefined
		) {
			const diff = Math.abs(expected.usdReceived - actual.usdReceived);
			const pctDiff =
				expected.usdReceived > 0 ? diff / expected.usdReceived : 0;
			usdMatch = pctDiff <= tolerance;

			details.push(
				`USD Received: Expected ${expected.usdReceived.toFixed(
					2
				)}, Got ${actual.usdReceived.toFixed(2)} (${
					usdMatch ? "✅" : "❌"
				})`
			);
		}
		...
		if (expected.avgPrice !== undefined && actual.avgPrice !== undefined) {
			...
			const pctDiff =
				expected.avgPrice > 0 ? diff / expected.avgPrice : 0;
			priceMatch = pctDiff <= tolerance;

			details.push(
				`Avg Price: Expected ${expected.avgPrice.toFixed(
					4
				)}, Got ${actual.avgPrice.toFixed(4)} (${
					priceMatch ? "✅" : "❌"
				})`
			);
		}
