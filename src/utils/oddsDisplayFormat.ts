...
export const VALID_ODDS_DISPLAY_STYLES = new Set<OddsDisplayStyle>(
	VALID_STYLES_LIST,
);
...
function hkToReducedFraction(hk: number, maxDenom = 1000): {
	num: number;
	den: number;
} | null {
...
export function formatOddsPrice(
	...
	switch (style) {
		...
		case "american":
			return layout === "dualWithCents"
				? formatAmericanWithCentsParen(p)
				: formatAmericanLabel(p);
		case "decimal":
		case "multiplier":
		case "fractional":
		case "percentage":
...
export function formatAvgOddsValue(
	p: number | null | undefined,
	style: OddsDisplayStyle,
): string {
	if (p === undefined || p === null || !Number.isFinite(p)) return "--";
