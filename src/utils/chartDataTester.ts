...
class ChartDataTester {
	...
	async testMarket(
		...
		if (!cachedMarket) {
			...
			result.recommendations.push(
				"Ensure market data is loaded via PredictionDataContext"
			);
		} else {
			console.log(
				`✅ Market found in cache: ${
					cachedMarket.displayName || cachedMarket.question
				}`
			);
		}
		...
		try {
			...
			result.expectedData.historicalCount = historicalPrices
			...
			if (historicalPrices && historicalPrices.length > 0) {
				...
				result.expectedData.firstHistoricalPrice =
					sortedPrices[0].price;
				result.expectedData.lastHistoricalPrice =
					sortedPrices[sortedPrices.length - 1].price;

				const firstDate = new Date(sortedPrices[0].timestamp * 1000);
				const lastDate = new Date(
					sortedPrices[sortedPrices.length - 1].timestamp * 1000
				);
				result.expectedData.historicalTimeRange = `${firstDate.toISOString()} to ${lastDate.toISOString()}`;

				console.log(
					`✅ Historical data found: ${historicalPrices.length} points`
				);
				console.log(
					`   First price: ${
						result.expectedData.firstHistoricalPrice
					} at ${firstDate.toLocaleString()}`
				);
				console.log(
					`   Last price: ${
						result.expectedData.lastHistoricalPrice
					} at ${lastDate.toLocaleString()}`
				);
			} else {
		...
		if (orderbook) {
			...
			if (orderbook.asks && orderbook.asks.length > 0) {
				result.orderbook.bestAsk = Math.min(
			...
			if (orderbook.bids && orderbook.bids.length > 0) {
				result.orderbook.bestBid = Math.max(
			...
			console.log(
				`📊 Orderbook: ${result.orderbook.asksCount} asks, ${result.orderbook.bidsCount} bids`
			);
		} else {
			console.log("❌ No orderbook data provided");
			result.issues.push("No orderbook data available");
			result.recommendations.push(
				"Ensure orderbook is loaded and passed to chart"
			);
		}
		...
		console.log(
			`📊 Would generate ${mockChartData.length} chart data points`
		);
		if (mockChartData.length > 0) {
			const firstPoint = mockChartData[0];
			const lastPoint = mockChartData[mockChartData.length - 1];
			console.log(
				`   First point: ${firstPoint.percentage}% at ${new Date(
					firstPoint.timestamp * 1000
				).toLocaleString()}`
			);
			console.log(
				`   Last point: ${lastPoint.percentage}% at ${new Date(
					lastPoint.timestamp * 1000
				).toLocaleString()}`
			);
		}
	...
	private validateResults(result: TestResult): void {
		...
		if (result.actualData.processedCount === 0) {
			result.issues.push(
				"Chart would be empty - no data points generated"
			);
			result.recommendations.push(
				"Implement proper fallback data generation"
			);
		}

		// Check if only showing fallback data when live data should be available
		if (
			result.orderbook.hasOrderbook &&
			result.orderbook.bestAsk !== null &&
			!result.expectedData.hasHistoricalData
		) {
			const wouldShowFallback = result.actualData.dataPoints.some(
				(p) => p.isFallback
			);
			if (wouldShowFallback) {
				result.recommendations.push(
					"Should transition from 50% fallback to live price"
				);
			}
		}
		...
		if (!result.actualData.cacheHit) {
			...
			result.recommendations.push(
				"Ensure PredictionDataContext pre-loads all market data"
			);
		}
		...
		if (
			result.orderbook.hasOrderbook &&
			result.orderbook.bestAsk === null &&
			result.orderbook.asksCount === 0
		) {
			result.issues.push(
				"Orderbook exists but has no asks - no live price available"
			);
			result.recommendations.push(
				"Check orderbook data quality and API responses"
			);
		}
	 ...
	 */
	async testAllMarketsInUmbrella(
		umbrellaId: string,
		markets: any[],
		...
		for (const market of markets) {
			...
			const marketName =
				market.displayName || market.question || "Unknown Market";
			const orderbook = orderbooks[questionId];

			if (questionId) {
				const result = await this.testMarket(
					questionId,
					marketName,
					orderbook
				);
	...
	private generateSummaryReport(results: TestResult[]): void {
		...
		const marketsWithHistoricalData = results.filter(
			(r) => r.expectedData.hasHistoricalData
		).length;
		const marketsWithOrderbooks = results.filter(
			(r) => r.orderbook.hasOrderbook
		).length;
		const marketsWithIssues = results.filter(
			(r) => r.issues.length > 0
		).length;
		const marketsInCache = results.filter(
			(r) => r.actualData.cacheHit
		).length;

		console.log(`📈 Markets tested: ${totalMarkets}`);
		console.log(
			`📦 Markets in cache: ${marketsInCache}/${totalMarkets} (${Math.round(
				(marketsInCache / totalMarkets) * 100
			)}%)`
		);
		console.log(
			`📊 Markets with historical data: ${marketsWithHistoricalData}/${totalMarkets} (${Math.round(
				(marketsWithHistoricalData / totalMarkets) * 100
			)}%)`
		);
		console.log(
			`📋 Markets with orderbooks: ${marketsWithOrderbooks}/${totalMarkets} (${Math.round(
				(marketsWithOrderbooks / totalMarkets) * 100
			)}%)`
		);
		console.log(
			`❌ Markets with issues: ${marketsWithIssues}/${totalMarkets} (${Math.round(
				(marketsWithIssues / totalMarkets) * 100
			)}%)`
		);
		...
		console.log(`\n⚡ Performance Insights:`);
		const avgHistoricalCount =
			results
				.filter((r) => r.expectedData.hasHistoricalData)
				.reduce((sum, r) => sum + r.expectedData.historicalCount, 0) /
				marketsWithHistoricalData || 0;

		console.log(
			`   Average historical data points per market: ${Math.round(
				avgHistoricalCount
			)}`
		);

		const marketsNeedingFallback = results.filter(
			(r) => !r.expectedData.hasHistoricalData && !r.orderbook.bestAsk
		).length;
		console.log(
			`   Markets needing pure fallback data: ${marketsNeedingFallback}`
		);
		...
		if (marketsInCache < totalMarkets) {
			console.log(
				`   1. Improve cache loading - ${
					totalMarkets - marketsInCache
				} markets missing from cache`
			);
		}
		if (marketsWithHistoricalData < totalMarkets * 0.5) {
			console.log(
				`   2. Investigate historical data loading - only ${Math.round(
					(marketsWithHistoricalData / totalMarkets) * 100
				)}% have data`
			);
		}
		if (marketsWithOrderbooks < totalMarkets) {
			console.log(
				`   3. Ensure all orderbooks are loaded - ${
					totalMarkets - marketsWithOrderbooks
				} missing orderbooks`
			);
		}
		if (marketsNeedingFallback > 0) {
			console.log(
				`   4. Implement robust 50% fallback for ${marketsNeedingFallback} markets with no data`
			);
		}
	}
	...
	exportResults(): any {
		return {
			...
			summary: {
				...
				marketsWithData: this.results.filter(
					(r) => r.expectedData.hasHistoricalData
				).length,
				marketsWithIssues: this.results.filter(
					(r) => r.issues.length > 0
				).length,
			},
