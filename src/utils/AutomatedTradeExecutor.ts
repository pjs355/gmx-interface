...
export class AutomatedTradeExecutor {
  ...
  static async executeTrade(
    ...
    const signature = await signer.signTypedData(domain, types, orderDataForSigning);
  ...
  private static calculateMarketBuyParams(
    ...
    for (const order of sortedOrders) {
      ...
      const costPerContract = position === 'no' ? (1 - order.price) : order.price;
      ...
      if (order.orders && Array.isArray(order.orders)) {
        totalAvailableSize = order.orders.reduce((sum, nestedOrder) => 
          sum + (nestedOrder.size || nestedOrder.makerQty || nestedOrder.origSize || 0), 0
        );
      } else {
  ...
  private static calculateMarketSellParams(
    ...
    return {
      minPrice: Math.round((minPriceSeen === Infinity ? 0 : minPriceSeen) * 100) / 100,
    };
  }
}
