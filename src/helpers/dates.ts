...
export function getTimestampByDaysAgo(daysAgo: number) {
	return (
		Math.floor(Date.now() / 1000 / 86400) * 86400 - daysAgo * 24 * 60 * 60
	);
}
...
export function normalizeDateRange<
	S extends Date | undefined,
	E extends Date | undefined
>(
	start: S,
	end: E
): [S extends Date ? number : undefined, E extends Date ? number : undefined] {
	const fromTxTimestamp = start
		? toSeconds(setTime(start, START_OF_DAY_DURATION))
		: undefined;
	const toTxTimestamp = end
		? toSeconds(setTime(end, INCLUDING_CURRENT_DAY_DURATION))
		: undefined;

	return [fromTxTimestamp, toTxTimestamp] as [
		S extends Date ? number : undefined,
		E extends Date ? number : undefined
	];
}
...
export function getYearStart() {
	return floorTimestamp(
		new Date(new Date().getUTCFullYear(), 0, 1, 0, 0, 0, 0).getTime() /
			1000,
		"day"
	);
}
