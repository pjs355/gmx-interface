...
export default function OrderbookDisplay({ 
  ...
  const flattenAndConsolidateOrders = (priceLevels: any[]): Array<{price: number, size: number, id: string}> => {
    const priceMap = new Map<number, {size: number, id: string}>();
    
    priceLevels?.forEach((level, levelIndex) => {
      if (level && typeof level === 'object' && level.price !== undefined) {
        const price = safeNumber(level.price);
        
        if (price > 0) {
          // Handle nested orders array structure
          if (level.orders && Array.isArray(level.orders)) {
            level.orders.forEach((order: any, orderIndex: number) => {
              if (order && typeof order === 'object') {
                const size = safeNumber(order.size || order.amount || 1);
                if (size > 0) {
                  if (priceMap.has(price)) {
                    ...
                    priceMap.set(price, {
                      size,
                      id: order.id || order.salt || `level-${levelIndex}-order-${orderIndex}`
                    });
                  }
                }
              }
            ...
            const size = safeNumber(level.size || level.amount || 1);
            if (size > 0) {
              if (priceMap.has(price)) {
                priceMap.get(price)!.size += size;
              } else {
                priceMap.set(price, {
                  size,
                  id: level.id || level.salt || `level-${levelIndex}`
                });
              }
            }
          }
  ...
  const sortedAsks = flattenedAsks.sort((a, b) => a.price - b.price).reverse();
  ...
  const invertedAsks = flattenedBids.map(bid => ({
    ...bid,
    price: 1 - bid.price, // Invert price: 0.6 becomes 0.4
    id: `inverted-${bid.id}`
  })).sort((a, b) => a.price - b.price).reverse(); // Sort ascending for asks, then reverse to show best ask at bottom
  
  const invertedBids = flattenedAsks.map(ask => ({
    ...ask,
  ...
  const bestAsk = displayAsks.length > 0 ? safeNumber(displayAsks[displayAsks.length - 1].price) : null;
  const bestBid = displayBids.length > 0 ? safeNumber(displayBids[0].price) : null;
  ...
  const calculateDepthPercentages = (orders: Array<{price: number, size: number, id: string}>, isAsks: boolean = false) => {
    ...
    if (isAsks) {
      ...
      const reversedOrders = ordersWithCumulative.reverse().map(order => {
        ...
        return { ...order, cumulativeTotal: reverseCumulativeTotal };
      });
    ...
    const maxCumulativeTotal = Math.max(...ordersWithCumulative.map(order => order.cumulativeTotal));
    
    // Calculate depth percentages based on cumulative totals
    return ordersWithCumulative.map(order => ({
      ...order,
      depthPercentage: maxCumulativeTotal > 0 ? (order.cumulativeTotal / maxCumulativeTotal) * 100 : 0
    }));
  };
            ...
            <div className="orderbook-tabs">
              <button
                ...
                style={isVsSingle ? {
                  ...
                  color: '#ffffff',
                  border: `2px solid ${activeTab === 'yes' ? yesColor : hexToRgba(yesColor, 0.35)}`,
                } : undefined}
              >
              ...
              <button
                ...
                style={isVsSingle ? {
                  ...
                  color: '#ffffff',
                  border: `2px solid ${activeTab === 'no' ? noColor : hexToRgba(noColor, 0.35)}`,
                } : undefined}
              >
                {noTeamLabel} {noLabel}
              </button>
