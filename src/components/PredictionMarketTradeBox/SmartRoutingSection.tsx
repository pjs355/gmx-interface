...
import { FlashingValue } from "@/components/FlashingValue/FlashingValue";
...
import { SHARE_SELL_COMPARE_EPS } from "@/features/trading/trade-box/checkBalances";
...
function sortVenuePreviews(
	...
	if (previews[0]!.side === "buy") return sortVenuePreviewsBuy(previews, predictFunFeeRateBps);
...
function splitBuyIsBestOrTied(
	...
	for (const p of previews ?? []) {
		...
		anyExecutable = true;
		maxSingle = Math.max(maxSingle, buyPreviewNetDisplayShares(p, predictFunFeeRateBps));
	}
...
function feeRouteFromBuyPreview(p: Extract<VenueRoutePreview, { side: "buy" }>): RoutePlan {
	return {
		side: "buy",
		legs: p.legs,
...
function visibleDrawerE2eLegAttrs(args: {
	...
	if (!args.selected || args.tradingVenue === "all" || args.orderType !== "market" || !args.leg) {
		return null;
	}
	const numShares =
		args.side === "buy"
...
function SmartRoutingLegRows({
	...
	return (
		<div className="smart-routing-drawer__legs">
			{legs.map((leg, idx) => {
				const displayShares =
					side === "buy" ? sorBuyPredictLegNetHeldShares(leg, predictFunFeeRateBps) : leg.shares;
				...
				const legGrossUsd =
					...
					leg.avgPrice > 0
						...
						: null;
				const atBuyDepthMax = side === "buy" && atMaxByVenue?.has(leg.venue) === true;
				...
				return (
					...
					>
						...
						<span className="smart-routing-drawer__leg-left">
							<span className="smart-routing-drawer__leg-line">
								...
								{atBuyDepthMax && showVenueLogo ? (
									<MaxBadgeInline measureKey={`${leg.venue}-${leg.shares}-${leg.avgPrice}`} />
								) : null}
						...
						<span className="smart-routing-drawer__leg-amount">
							{legGrossUsd != null ? `$ ${formatSorBuyCostUsdDisplay(legGrossUsd)}` : ""}
						</span>
					</div>
				);
			})}
...
export default function SmartRoutingSection({
	...
	if (prevMarketKeyRef.current !== null && prevMarketKeyRef.current !== marketKeyForSticky) {
		stableDisplayRouteRef.current = null;
		stableVenuePreviewsRef.current = null;
	}
	...
	const liveDisplayRoute: RoutePlan | null =
		rawDisplayRoute && rawDisplayRoute.side === expectedSide ? rawDisplayRoute : null;
	const liveVenuePreviews: VenueRoutePreview[] | null =
		rawVenuePreviews == null
			? null
			: rawVenuePreviews.length === 0 || rawVenuePreviews[0]!.side === expectedSide
				? rawVenuePreviews
				: null;
	...
	const splitMetricsPending = Boolean(trustCtx && routePreviewAllowed && !omnibusMetricsTrusted);
	...
	useEffect(() => {
		...
		const allowed = new Set(sortedVenuePreviews.map((p) => sorVenueToTradingVenue(p.venue)));
		...
		const timer = window.setTimeout(() => {
			...
			const allowedNow = new Set(previews.map((p) => sorVenueToTradingVenue(p.venue)));
			if (allowedNow.has(tv)) return;
			onSelectVenue(sorVenueToTradingVenue(previews[0]!.venue));
		}, VENUE_IMPOSSIBLE_TAB_DEBOUNCE_MS);

		return () => window.clearTimeout(timer);
	}, [venueSelectionLocked, routePreviewAllowed, sortedVenuePreviews, tradingVenue, onSelectVenue]);
	...
	const effectiveSide: SorSide = side ?? displayRoute?.side ?? executionRoute?.side ?? "buy";
		...
		>
			{hasAnyRow && (
				<div className="smart-routing-section__headers">
					...
					<span className="smart-routing-section__header-label">{rightHeaderLabel}</span>
				</div>
			)}
									...
									{splitBuyNetDisplay > 0 && (
										<span className="smart-routing-row__sub">
											{splitMetricsPending ? (
												...
												<>{formatLegAvg(displayRoute.totalCost / splitBuyNetDisplay)} avg.</>
											)}
										</span>
									)}
								</div>
			...
			{sortedVenuePreviews?.map((preview) => {
				const key = preview.side === "buy" ? `buy-${preview.venue}` : `sell-${preview.venue}`;
				const selected =
					tradingVenue === sorVenueToTradingVenue(preview.venue) ||
					(omnibusHighlightsTopVenueRow &&
						sortedVenuePreviews &&
						preview === sortedVenuePreviews[0]);
				const letter = (VENUE_DISPLAY_NAMES[preview.venue] ?? "?").slice(0, 1).toUpperCase();
							...
							{open && (
								<div className="smart-routing-drawer">
									...
									<div className="smart-routing-drawer__footer">
										...
										>
											<span>Est. receive</span>
											<span>$ {formatSorSellProceedsUsdDisplay(displayProceeds)}</span>
										</div>
									</div>
								</div>
							)}
				...
				const netSharesForAvg = overlayRoute
					...
					: previewNetShares;
				const displayAvgPrice =
					netSharesForAvg > 0 && Number.isFinite(costForAvg) ? costForAvg / netSharesForAvg : null;
								...
								<div className="smart-routing-row__left">
									...
									<div className="smart-routing-row__meta">
										<span className="smart-routing-row__name">
											...
											{kalshiNeedsKyc ? (
												<FiLock className="smart-routing-row__name-lock" aria-hidden />
											) : theoretical ? (
												" (book only)"
											) : null}
										</span>
						...
						{open && (
							<div className="smart-routing-drawer">
								<SmartRoutingLegRows
									...
									side="buy"
									formatLegAvg={formatLegAvg}
									atMaxByVenue={p.insufficientLiquidity ? new Set([p.venue]) : undefined}
									predictFunFeeRateBps={predictFunFeeRateBps}
									e2eLegAttrs={e2eLegAttrs}
								/>
								<div className="smart-routing-drawer__footer">
