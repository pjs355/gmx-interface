...
import type { TradeBoxCoreState, TradeBoxProps } from "@/features/trading/trade-box/types";
...
export default function TradeBoxExecutionFooter(props: TradeBoxExecutionFooterProps) {
	...
	return (
		<>
			{(() => {
				const route = tradingVenue === "all" ? sorRoute.displayRoute : sorRoute.executionRoute;
			...
			{side === "sell" &&
				...
				(() => {
					...
					return Number.isFinite(n) && n > maxScopedSellShares + SHARE_SELL_COMPARE_EPS;
				})() && (
				...
				onClick={() => {
					try {
						mixpanelTrack("TradeButtonClicked", {
							...
							amount: amount,
							price: price,
							limitPriceProb: orderType === "limit" && price ? Number(price) / 100 : null,
							derivedAvgFillPriceFromBook:
								orderType === "market" && oddsData ? oddsData.avgPrice : null,
							derivedAvgFillCents:
								orderType === "market" && oddsData ? Math.round(oddsData.avgPrice * 100) : null,
							marketSellAvgCents: orderType === "market" && side === "sell" ? sellAvgCents : null,
						...
						style={{
							...
							backgroundColor:
								sorExecution.execution.status === "partial"
									...
									: "rgba(239, 68, 68, 0.08)",
							color: sorExecution.execution.status === "partial" ? "#f59e0b" : "#ef4444",
						}}
					>
						{sorExecution.execution.status === "partial" && (
							<div
							...
							>
								<span>
									{side === "sell" ? "Partially sold" : "Partially filled"}:{" "}
									{formatSorDetailsSharesDisplay(sorExecution.execution.totalFilledShares)} shares
								</span>
								<div style={{ display: "flex", gap: 8 }}>
									...
									>
										Re-route{" "}
										{sorExecution.remainingBudget != null
											? `$${sorExecution.remainingBudget.toFixed(2)}`
											: "remaining"}
									</button>
									<button
										type="button"
						...
						{sorExecution.execution.status === "failed" && (
							...
							>
								<span>
									{side === "sell"
										? "Execution failed. Shares remain in your accounts."
      ...
      */}
			{orderResult && (
				<div
					...
					data-qa-fill-error={orderResult.success ? undefined : orderResult.error || ""}
