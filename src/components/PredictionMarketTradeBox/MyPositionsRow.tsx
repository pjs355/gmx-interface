...
import type { TradeBoxShareBalancesSnapshot } from "@/features/trading/trade-box/hooks/useTradeBoxShareBalances";
import type { MatchedMarket } from "@/types/odds-monitor";
import type { TradingVenue } from "@/features/trading/trade-box/types";
import {
	formatShareCountDataQa,
...
export function MyPositionsRow({
	...
	side,
	selectedPosition,
	matchedMonitor: _matchedMonitor,
	shareBalances,
	...
	if (side === "buy") {
		const pendingEmptyPosition = positionSharesRefreshing && buyLines.length === 0;
		...
		const pendingOutcomeLabel = selectedPosition === "no" ? noTeamLabel : yesTeamLabel;
		const pendingLineColor = colorForLine(selectedPosition === "no" ? "no" : "yes");
				...
				>
					{pendingEmptyPosition ? (
						...
						buyLines.map((line) => {
							...
							const showBuyDetails = breakdown.length > 1 && !positionSharesRefreshing;
							...
							const headlineBlock = showBuyDetails ? (
								<button
									...
									aria-label={
										lineDetailsOpen ? "Hide position breakdown" : "Show position breakdown"
									}
									style={{
										display: "inline-flex",
										alignItems: "center",
								...
								>
									<span
										style={{
											fontSize: 10,
											color: "#ffffff",
											transform: lineDetailsOpen ? "rotate(180deg)" : "rotate(0deg)",
									...
									<span style={{ color: lineHeadlineColor }}>{headlineContent}</span>
								</button>
							) : (
								<div
									...
									{showBuyDetails && lineDetailsOpen ? (
										...
										>
											{breakdown.map((row) => (
												...
												>
													{formatShareCountDisplay(row.shares)} Shares {line.label} (
													{row.venueDisplay})
												</div>
											))}
										</div>
	...
	// Sell
	const headlineColor = selectedPosition === "no" ? colorForLine("no") : colorForLine("yes");
	...
	if (sellDualBothSides) {
		const pendingEmptyDual = positionSharesRefreshing && buyLines.length === 0;
			...
			>
				...
				>
					{pendingEmptyDual ? (
						...
						buyLines.map((line) => {
							const breakdown = buyVenueBreakdownByOutcome[line.side];
							const showSellDualDetails = breakdown.length > 1 && !positionSharesRefreshing;
	...
	const showDetails = sellVenueBreakdown.length > 1 && !positionSharesRefreshing;

	const headlineBlock = showDetails ? (
		<button
			type="button"
			onClick={() => setDetailsOpen((o) => !o)}
			data-qa="my-positions-row-details-toggle"
			aria-expanded={detailsOpen}
			aria-label={detailsOpen ? "Hide position breakdown" : "Show position breakdown"}
			style={{
				display: "inline-flex",
				alignItems: "center",
				justifyContent: "flex-end",
					...
					{showDetails && detailsOpen ? (
						...
						>
							{sellVenueBreakdown.map((row) => (
								...
								>
									{formatShareCountDisplay(row.shares)} Shares {sellOutcomeLabel} (
									{row.venueDisplay})
								</div>
							))}
						</div>
