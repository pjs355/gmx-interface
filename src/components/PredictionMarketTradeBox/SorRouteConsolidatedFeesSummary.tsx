...
import type { RoutePlan, RouteLeg, SorVenue } from "@/features/trading/sor";
import {
	VENUE_DISPLAY_NAMES,
...
export function aggregateFeesByVenue(legs: RouteLeg[]): { venue: SorVenue; fee: number }[] {
	const map = new Map<SorVenue, number>();
	for (const leg of legs) {
		map.set(leg.venue, (map.get(leg.venue) ?? 0) + leg.fee);
	}
	return [...map.entries()].filter(([, fee]) => fee > 0).map(([venue, fee]) => ({ venue, fee }));
}
