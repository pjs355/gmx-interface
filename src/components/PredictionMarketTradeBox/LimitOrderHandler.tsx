...
export function useLimitOrderHandler(orderbook: OrderbookSnapshot | null) {
	...
	const validateLimitOrder = useCallback(
		(
			amount: string,
			price: string,
			position: "yes" | "no",
	...
	const isImmediatelyExecutable = useCallback(
		(price: string, position: "yes" | "no"): boolean => {
			...
			if (position === "yes") {
				...
				const bestAsk =
					orderbook.asks?.length > 0 ? Math.min(...orderbook.asks.map((ask) => ask.price)) : 1;
				return numPrice >= bestAsk;
			} else {
				// For NO position, check if price is at or above best ask (inverted)
				const bestAsk =
					orderbook.asks?.length > 0 ? Math.min(...orderbook.asks.map((ask) => 1 - ask.price)) : 1;
				return numPrice >= bestAsk;
			}
		},
		[orderbook],
