  ...
  const executeTrade = useCallback(
    async (
      ...
      try {
        ...
        if (!params.market) {
          throw new Error(`CRITICAL ERROR: No market data provided. Trade cannot proceed.`);
        }

        if (!params.market.yesTokenId || !params.market.noTokenId) {
          throw new Error(`CRITICAL ERROR: Missing token IDs in market data. yesTokenId=${params.market.yesTokenId}, noTokenId=${params.market.noTokenId}. Trade cannot proceed.`);
        }

        // CRITICAL: Validate position parameter
        if (params.position !== 'yes' && params.position !== 'no') {
          throw new Error(`CRITICAL ERROR: Invalid position: ${params.position}. Must be 'yes' or 'no'.`);
        }

        // SINGLE SOURCE OF TRUTH: Determine expected token ID from market data only
        const expectedTokenId = params.position === 'yes' ? params.market.yesTokenId : params.market.noTokenId;
        console.log("🎯 SINGLE SOURCE OF TRUTH - Token ID validation:", {
          position: params.position,
        ...
        console.log("🚨 FINAL ADDRESS CHECK BEFORE ORDER CREATION:", {
          ...
          "embeddedWalletAddress (should be embedded)": embeddedWalletAddress,
          "addresses are different": params.userAddress !== embeddedWalletAddress
        });
        ...
        console.log("🔍 Address verification:", {
          ...
          "addresses match": orderData.maker === params.userAddress && orderData.signer === embeddedWalletAddress
        });
        ...
        if (!isCorrectTokenId) {
          throw new Error(`TOKEN ID MISMATCH! Position: ${params.position}, Expected: ${expectedTokenId}, Actual: ${actualTokenId}`);
        }

        console.log("✅ TOKEN ID VALIDATION PASSED - Correct token ID used");
        ...
        console.log("🔐 Signer ready:", {
          ...
          hasSignTypedData: typeof activeSigner.signTypedData === "function",
        });
        ...
        try {
          ...
          console.warn("⚠️ Failed to fetch on-chain nonce, will use service-provided value:", e);
        }
        ...
        console.log("🔍 CRITICAL: Side comparison - Signing vs API:", {
          ...
          correctSigning: typeof orderDataForSigning.side === 'number' && (orderDataForSigning.side === 0 || orderDataForSigning.side === 1),
          correctApi: typeof params.side === 'string' && (params.side === 'buy' || params.side === 'sell')
        });

        console.log("🔐 COMPLETE SIGNING STRUCTURE (NUMERIC VALUES ONLY):", JSON.stringify(orderDataForSigning, null, 2));
        
        console.log("🔍 DETAILED SIGNING ANALYSIS:", {
          "orderData.numericSide": orderData.numericSide,
          "typeof orderData.numericSide": typeof orderData.numericSide,
          "orderDataForSigning.side": orderDataForSigning.side,
          "typeof orderDataForSigning.side": typeof orderDataForSigning.side,
        ...
        console.log("🌐 COMPLETE API PAYLOAD (STRING VALUES):", JSON.stringify(signedOrder, null, 2));

        // Submit the signed order to your local server
        console.log("🌐 Submitting signed order to server...");
        console.log("📤 Full order payload being sent:", JSON.stringify(signedOrder, null, 2));
        ...
        const apiResult = await predictionMarketService.submitOrderToAPI(signedOrder, params.marketId);
