...
type ViteEnvironmentMode = "production" | "testnet" | "local-production";
...
rl.question(
	`${COLORS.magenta}Enter choice (1, 2, or 3): ${COLORS.reset}`,
	(answer) => {
...
`);
		...
		rl.question(
			`${COLORS.cyan}Use Railway proxy? (y/n): ${COLORS.reset}`,
			(proxyAnswer) => {
				rl.close();
				...
				if (useProxy) {
					console.log(
						`${COLORS.cyan}✓ Railway proxy: CLOB + Predict orders (LIVE; TEST/DEV if tunnel URL set)${COLORS.reset}\n`
					);
				} else {
					console.log(
						`${COLORS.dim}✓ Direct routing (no Railway proxy)${COLORS.reset}\n`
					);
				}
