...
import { getEntryPrice, getLeverage, getLiquidationPrice, getPositionKey, getPositionNetValue, getPositionPnlUsd, } from "utils/positions";
...
export class Positions extends Module {
    ...
    async getPositionsInfo(p) {
        ...
        const positionsInfoData = Object.keys(positionsData).reduce((acc, positionKey) => {
            ...
            const closingPriceImpactDeltaUsd = getPriceImpactForPosition(marketInfo, position.sizeInUsd * -1n, position.isLong, { fallbackToZero: true });
            const positionFeeInfo = getPositionFee(marketInfo, position.sizeInUsd, closingPriceImpactDeltaUsd > 0, userReferralInfo, uiFeeFactor);
