...
describe("UI token permit configs", () => {
    SUPPORTED_CHAIN_IDS.forEach(async (chainId) => {
        ...
        it(`tokens isPermitSupported should be consistent with contracts for ${getChainName(chainId)}`, async () => {
            ...
            tokens.forEach((token, index) => {
                ...
                if (supportsPermit !== Boolean(token.isPermitSupported)) {
                    errors.push(`${getChainName(chainId)} ${token.symbol} isPermitSupported should be ${supportsPermit}, address: ${token.address}, error: ${result.error?.message.slice(0, 100) || "none"}`);
                }
            });
            if (errors.length > 0) {
        ...
        it(`tokens with permit support should have required methods for ${getChainName(chainId)}`, async () => {
            const tokens = getV2Tokens(chainId).filter((token) => !token.isNative && !token.isSynthetic && token.isPermitSupported);
            ...
            tokens.forEach((token, tokenIndex) => {
                ...
                if (missingMethods.length > 0) {
                    errors.push(`${getChainName(chainId)} ${token.symbol} (${token.address}) is marked as permit supported but missing methods: ${missingMethods.join(", ")}`);
                }
            });
            if (errors.length > 0) {
                throw new Error(errors.join("\n"));
