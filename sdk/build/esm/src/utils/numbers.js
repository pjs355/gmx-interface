...
export function formatBalanceAmount(amount, tokenDecimals, tokenSymbol, { showZero = false, toExponential = true, isStable = false, } = {}) {
    if (amount === undefined)
        return "-";
    if (amount === 0n) {
        if (showZero === true) {
    ...
    if (absAmountFloat >= 1)
        value = formatAmount(amount, tokenDecimals, baseDecimals, true);
    else if (absAmountFloat >= 0.1)
        value = formatAmount(amount, tokenDecimals, baseDecimals + 1, true);
    else if (absAmountFloat >= 0.01)
        value = formatAmount(amount, tokenDecimals, baseDecimals + 2, true);
    else if (absAmountFloat >= 0.001)
        value = formatAmount(amount, tokenDecimals, baseDecimals + 3, true);
